...
struct SaveFormat {
    SaveFormat(const Glib::ustring &_format, int _png_bits, int _jpeg_quality,
               ...
               bool _tiff_uncompressed, bool _save_params)
        : format(_format), pngBits(_png_bits), jpegQuality(_jpeg_quality),
          jpegSubSamp(_jpeg_sub_samp), tiffBits(_tiff_bits),
          tiffFloat(_tiff_float), tiffUncompressed(_tiff_uncompressed),
          saveParams(_save_params)
    {
    }
    SaveFormat(const Glib::ustring &_format, int _png_bits, int _tiff_bits,
               bool _tiff_float)
        : SaveFormat(_format, _png_bits, 90, 2, _tiff_bits, _tiff_float, true,
                     true)
    {
    }
...
public:
    ...
    static constexpr const char *THEMEREGEXSTR =
        "^(.+)-GTK3-(\\d{1,2})?_(\\d{1,2})?(-DEPRECATED)?\\.css$";
...
public:
    ...
    Glib::ustring
        profilePath; // can be an absolute or relative path; depending on this
    ...
    int thumbSizeTab;
    int thumbSizeQueue;
    bool sameThumbSize; // Will use only one thumb size for the file browser and
                        // the single editor tab, and avoid recomputing them
    ...
    EditorOutDir
        editor_out_dir; // output directory for "open in external editor"
    ...
    std::vector<Glib::ustring>
        parseExtensions; // List containing all extensions type
    std::vector<int>
        parseExtensionsEnabled; // List of bool to retain extension or not
    std::vector<Glib::ustring>
        parsedExtensions; // List containing all retained extensions (lowercase)
    std::set<std::string> parsedExtensionsSet; // Set containing all retained
    ...
    std::vector<double>
        cutOverlayBrush; // Red;Green;Blue;Alpha , all ranging 0..1
    std::vector<double>
        navGuideBrush; // Red;Green;Blue;Alpha , all ranging 0..1

    Glib::ustring sndBatchQueueDone;
    Glib::ustring sndLngEditProcDone;
    double sndLngEditProcDoneSecs; // Minimum processing time seconds till the
                                   // sound is played
    bool sndEnable;

    enum {
        HISTOGRAM_POS_OFF = 0,
        HISTOGRAM_POS_LEFT = 1,
        HISTOGRAM_POS_RIGHT = 2
    };
    ...
    int rgbDenoiseThreadLimit; // maximum number of threads for the denoising
                               // tool ; 0 = use the maximum available
    ...
    size_t maxRecentFolders; // max. number of recent folders stored in options
                             // file
    std::vector<Glib::ustring>
        recentFolders; // List containing all recent folders
    ...
    int adjuster_shortcut_scrollwheel_factor; // to control the adjustment step
                                              // when using tool shortcuts with
                                              // the mouse wheel
    ...
    enum class PreviewResamplingQuality { LOW, MEDIUM, HIGH };
