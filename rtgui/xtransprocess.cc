                        ...
                        true)
{
    ...
    for (const auto method_string :
         RAWParams::XTransSensor::getMethodStrings()) {
...
void XTransProcess::read(const rtengine::procparams::ProcParams *pp)
{
    ...
    for (size_t i = 0; i < RAWParams::XTransSensor::getMethodStrings().size();
         ++i)
...
void XTransProcess::write(rtengine::procparams::ProcParams *pp)
{
    ...
    pp->raw.xtranssensor.dualDemosaicContrast =
        dualDemosaicContrast->getValue();
    ...
    if (currentRow >= 0 &&
        ...
        pp->raw.xtranssensor.method =
            procparams::RAWParams::XTransSensor::Method(currentRow);
    }
}
...
void XTransProcess::methodChanged()
{
    ...
    if (listener && method->get_active_row_number() >= 0) {
        listener->panelChanged(
            currentMethod == RAWParams::XTransSensor::Method::MONO ||
