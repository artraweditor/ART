...
struct LocaleToLang
    ...
    key(const Glib::ustring &major,
        const Glib::ustring &minor = Glib::ustring())
    {
...
void setGtkLanguage(const Glib::ustring &language)
{
    if (language != "default") { // nothing to change when using default
        ...
        // On MacOS, LANG environment variable is not defined when running app
        // bundle So we should set all locale data
        const Glib::ustring localeUTF8 = lang + ".UTF-8";

        lang =
            lang +
            ".UTF-8"; // According to Apple documentation, UTF-8 is a built-in
        ...
        if (env_langc) {
            ...
            if (!env_lang.empty()) {
                const std::string::size_type suffix_pos =
                    env_lang.find_first_of(".");
    ...
    for (const auto &fname : fnames) {
        ...
        while (std::getline(file, entry)) {
            ...
            if (std::getline(line, key, ';') &&
                ...
                hint = translations.emplace_hint(hint, std::move(key),
                                                 std::move(value));
            }
        }
    }
...
Glib::ustring MultiLangMgr::getOSUserLanguage()
{
    ...
    const int langLen =
        GetLocaleInfo(localeID, LOCALE_SISO639LANGNAME, localeName, 9);
    if (langLen <= 0) {
        return langName;
    }
    ...
    const int countryLen = GetLocaleInfo(localeID, LOCALE_SISO3166CTRYNAME,
                                         &localeName[langLen], 9);
    if (countryLen <= 0) {
        return langName;
    }
    ...
    // "LANG" environment variable is not defined. Retrieving it from
    // CoreFundation API Get used Mac string encoding
    ...
    if (langCodeStr != NULL) {
        ...
        CFStringGetCString(langCodeStr, langCodeBuffer, langCodeStrLength,
                           strEncoding);
    ...
    if (countryCodeStr != NULL) {
        ...
        CFStringGetCString(countryCodeStr, countryCodeBuffer,
                           countryCodeStrLength, strEncoding);
