...
ThresholdSelector::ThresholdSelector(double minValue, double maxValue,
                                     double defBottom, double defTop,
...
ThresholdSelector::ThresholdSelector(double minValue, double maxValue,
                                     double defBottomLeft, double defTopLeft,
                                     double defBottomRight, double defTopRight,
                                     unsigned int precision, bool startAtOne)
    : coloredBar(RTO_Left2Right)
{
 ...
 * Set the position of the sliders without telling it to the listener
 */
void ThresholdSelector::setPositions(double bottomLeft, double topLeft,
                                     double bottomRight, double topRight)
{

    bool different = ((positions[TS_TOPLEFT] != topLeft) ||
...
void ThresholdSelector::setDefaults(double bottomLeft, double topLeft,
                                    double bottomRight, double topRight)
{
...
void ThresholdSelector::updateBackBuffer()
{
    ...
    double verticalSliderPadding = std::floor(
        ((double)h - (double)padding.get_top() - (double)padding.get_bottom()) *
    ...
    double innerBarY =
        verticalSliderPadding + 1. * s + (double)padding.get_top();
    double innerBarW = (double)w - innerBarX - (double)padding.get_right() -
                       hwslider - 0.5 * s;
    double innerBarH = (double)h - innerBarY - verticalSliderPadding - 1. * s -
                       (double)padding.get_bottom();
    if (is_sensitive() && coloredBar.canGetColors()) {
        if (updatePolicy == RTUP_DYNAMIC) {
            coloredBar.setDirty(true);
        }
        // this will eventually create/update the off-screen Surface for the
        // gradient area only !
    ...
    if (bgCurveProvider) {
        ...
        std::vector<double> pts = bgCurveProvider->getCurvePoints(
            this); // the values sent by the provider are not checked (assumed
                   // to be correct)
        ...
        if (!separatedSliders) {
            ...
            if (positions[p[1]] >
                minValTop) { // we use minValTop since if this block is
                             // executed, it means that we are in a simple
                             // Threshold where both bottom and top range are
                             // the same
    ...
    style->render_frame(cr, innerBarX - 1. * s, innerBarY - 1. * s,
                        innerBarW + 2. * s, innerBarH + 2. * s);
    ...
    for (int i = 0; i < (doubleThresh ? 4 : 2); ++i) {
        ...
        double baseY = i == 0 || i == 2
                           ? (double)h - (double)padding.get_bottom() - 0.5 * s
                           : (double)padding.get_top() + 0.5 * s;

        style->render_slider(cr, posX - hwslider,
                             i == 0 || i == 2 ? arrowY : baseY, wslider,
                             i == 0 || i == 2 ? baseY - arrowY : arrowY - baseY,
...
void ThresholdSelector::findLitCursor(int posX, int posY)
{
    ...
    if (posY >= 0 && posY <= h / 2) {
        if (posX >= (int)(innerBarX - hwslider) &&
            posX <= (int)(innerBarX + innerBarW + hwslider)) {
            litCursor = TS_TOPLEFT;

            if (doubleThresh) {
                // we use minValTop since if this block is executed, it means
                // that we are in a simple Threshold where both bottom and top
                // range are the same
                double cursorX = ((double)posX - innerBarX) *
    ...
    if (!active_bot_ &&
        (litCursor == TS_BOTTOMLEFT || litCursor == TS_BOTTOMRIGHT)) {
        litCursor = TS_UNDEFINED;
    }
}
...
void ThresholdSelector::findBoundaries(double &min, double &max)
{
    ...
    case (TS_BOTTOMRIGHT):
        if (initalEq1) {
            ...
            max = secondaryMovedCursor == TS_UNDEFINED
                      ? positions[TS_TOPRIGHT]
                      : maxValTop - (positions[TS_TOPRIGHT] -
                                     positions[TS_BOTTOMRIGHT]);
        } else {
            min = secondaryMovedCursor == TS_UNDEFINED
                      ? positions[TS_TOPRIGHT]
                      : positions[TS_TOPLEFT] + (positions[TS_BOTTOMRIGHT] -
                                                 positions[TS_TOPRIGHT]);
            max = maxValTop;
        }
    ...
    case (TS_TOPRIGHT):
        if (initalEq1) {
            min = secondaryMovedCursor == TS_UNDEFINED
                      ? positions[TS_BOTTOMRIGHT]
                      : positions[TS_BOTTOMLEFT] + (positions[TS_TOPRIGHT] -
                                                    positions[TS_BOTTOMRIGHT]);
            max = maxValTop;
        } else {
            ...
            max = secondaryMovedCursor == TS_UNDEFINED
                      ? positions[TS_BOTTOMRIGHT]
                      : maxValTop - (positions[TS_BOTTOMRIGHT] -
                                     positions[TS_TOPRIGHT]);
        }
...
double ThresholdSelector::to01(ThreshCursorId cursorId)
{
    ...
    if (cursorId == TS_BOTTOMLEFT || cursorId == TS_BOTTOMRIGHT) {
        rVal = (positions[cursorId] - minValBottom) /
               (maxValBottom - minValBottom);
    } else {
...
void ThresholdSelector::updateTooltip()
{
    ...
    if (doubleThresh) {
        Glib::ustring t_top = Glib::ustring::compose(
            "<b>%1:</b> %2     <b>%3:</b> %4", M("THRESHOLDSELECTOR_TL"),
        ...
        Glib::ustring t_bot = Glib::ustring::compose(
            "<b>%1:</b> %2     <b>%3:</b> %4", M("THRESHOLDSELECTOR_BL"),
            Glib::ustring::format(std::fixed,
                                  std::setprecision(precisionBottom),
                                  positions[TS_BOTTOMLEFT]),
        ...
        Glib::ustring t_top = Glib::ustring::compose(
            "<b>%1:</b> %2", separatedLabelTop,
            Glib::ustring::format(std::fixed, std::setprecision(precisionTop),
                                  positions[TS_TOPLEFT]));
        Glib::ustring t_bot = Glib::ustring::compose(
            "<b>%1:</b> %2", separatedLabelBottom,
            Glib::ustring::format(std::fixed,
                                  std::setprecision(precisionBottom),
                                  positions[TS_BOTTOMLEFT]));
        if (active_top_) {
        ...
        Glib::ustring t_top = Glib::ustring::compose(
            "<b>%1:</b> %2", M("THRESHOLDSELECTOR_T"),
            Glib::ustring::format(std::fixed, std::setprecision(precisionTop),
                                  positions[TS_TOPLEFT]));
        Glib::ustring t_bot = Glib::ustring::compose(
            "<b>%1:</b> %2", M("THRESHOLDSELECTOR_B"),
            Glib::ustring::format(std::fixed,
                                  std::setprecision(precisionBottom),
                                  positions[TS_BOTTOMLEFT]));
        if (active_top_) {
