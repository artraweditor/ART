...
void setprogressStrUI(double val, const Glib::ustring str,
...
private:
    void prepareProfileBox()
    {
        if (rtengine::Settings::color_mgmt_mode ==
            ...
            // if (find_default_monitor_profile
            // (profileBox.get_root_window()->gobj(), defprof, defprofname)) {
            if (getSystemDefaultMonitorProfile(
                    profileBox.get_root_window()->gobj(), defprof,
                    defprofname)) {
                profileBox.append(M("MONITOR_PROFILE_SYSTEM") + " (" +
                                  defprofname + ")");
    ...
    void prepareIntentBox()
    {
        if (rtengine::Settings::color_mgmt_mode ==
            ...
            intentBox.addEntry("intent-relative.svg",
                               M("PREFERENCES_INTENT_RELATIVE"));
            intentBox.addEntry("intent-absolute.svg",
                               M("PREFERENCES_INTENT_ABSOLUTE"));
            setExpandAlignProperties(intentBox.buttonGroup, false, false,
                                     Gtk::ALIGN_CENTER, Gtk::ALIGN_FILL);
    ...
    void prepareSoftProofingBox()
    {
        Gtk::Image *softProofImage =
            Gtk::manage(new RTImage("gamut-softproof.svg"));
        ...
        Gtk::Image *spGamutCheckImage =
            Gtk::manage(new RTImage("gamut-warning.svg"));
        ...
        spGamutCheckImage =
            Gtk::manage(new RTImage("gamut-warning-monitor.svg"));
        spGamutCheckImage->set_padding(0, 0);
        spGamutCheckMonitor.add(*spGamutCheckImage);
        spGamutCheckMonitor.set_relief(Gtk::RELIEF_NONE);
        spGamutCheckMonitor.set_tooltip_markup(
            M("SOFTPROOF_GAMUTCHECK_MONITOR_TOOLTIP"));
        ...
        if (active && !show_dialog) {
            ...
            if (show_dialog) {
                Gtk::FileChooserDialog dialog(getToplevelWindow(parent),
                                              M("PREFERENCES_PRTPROFILE"),
                                              Gtk::FILE_CHOOSER_ACTION_OPEN);
                bindCurrentFolder(dialog,
                                  options.rtSettings.monitorIccDirectory);
            ...
            if (!prof) {
                ...
                parent->error(Glib::ustring::compose(
                    M("ERROR_MSG_INVALID_PROFILE"), name));
                return true;
            } else {
        ...
        if (rtengine::Settings::color_mgmt_mode ==
            ...
            if (profileBox.get_active_row_number() == 0) {
                ...
                const bool supportsPerceptual =
                    supportedIntents & 1 << INTENT_PERCEPTUAL;
                ...
                if (supportsPerceptual || supportsRelativeColorimetric ||
                    ...
                    intentBox.setItemSensitivity(1,
                                                 supportsRelativeColorimetric);
                    intentBox.setItemSensitivity(2,
                                                 supportsAbsoluteColorimetric);
...
public:
        ...
        : intentBox(Glib::ustring(), true), processor(ipc), parent(p)
    {
        ...
        // softproofConn =
        softProof.signal_toggled().connect(
            sigc::mem_fun(this, &ColorManagementToolbar::softProofToggled));
        softproofConn = softProof.signal_button_release_event().connect(
            sigc::mem_fun(*this, &ColorManagementToolbar::softProofPressed),
            false);
    ...
    void reset()
    {
        if (rtengine::Settings::color_mgmt_mode ==
            ...
            if (!defprof.empty() && options.rtSettings.autoMonitorProfile) {
                ...
                setActiveTextOrIndex(profileBox,
                                     options.rtSettings.monitorProfile, 0);
            }
...
EditorPanel::EditorPanel(FilePanel *filePanel)
    : catalogPane(nullptr), realized(false), tbBeforeLock(nullptr),
      iHistoryShow(nullptr), iHistoryHide(nullptr), iTopPanel_1_Show(nullptr),
      iTopPanel_1_Hide(nullptr), iRightPanel_1_Show(nullptr),
      iRightPanel_1_Hide(nullptr), iBeforeLockON(nullptr),
      iBeforeLockOFF(nullptr), previewHandler(nullptr),
      beforePreviewHandler(nullptr), beforeIarea(nullptr), beforeBox(nullptr),
      afterBox(nullptr), beforeLabel(nullptr), afterLabel(nullptr),
      beforeHeaderBox(nullptr), afterHeaderBox(nullptr), parent(nullptr),
      parentWindow(nullptr), openThm(nullptr), selectedFrame(0), isrc(nullptr),
      ipc(nullptr), beforeIpc(nullptr), err(0), isProcessing(false),
      histogram_observable(nullptr), histogram_scope_type(ScopeType::NONE)
{
    ...
    toggleHistogramProfile->set_tooltip_markup(
        (M("PREFERENCES_HISTOGRAM_TOOLTIP")));
    ...
    toolBarPanel->pack_end(*iareapanel->imageArea->indClippedPanel,
                           Gtk::PACK_SHRINK, 0);
    toolBarPanel->pack_end(*vsepz, Gtk::PACK_SHRINK, 2);
    toolBarPanel->pack_end(*iareapanel->imageArea->previewModePanel,
                           Gtk::PACK_SHRINK, 0);
    ...
    setExpandAlignProperties(saveimgas, false, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_FILL);
    ...
    setExpandAlignProperties(queueimg, false, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_FILL);

    Gtk::Image *sendToEditorButtonImage =
        Gtk::manage(new RTImage("palette-brush.svg"));
    ...
    setExpandAlignProperties(sendtogimp, false, false, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_FILL);
    ...
    setExpandAlignProperties(tbShowHideSidePanels, false, false,
                             Gtk::ALIGN_CENTER, Gtk::ALIGN_FILL);
    ...
    // Adding widgets from center to the left, on the left side (using
    // Gtk::POS_LEFT)
    ...
    iops->attach_next_to(*iareapanel->imageArea->zoomPanel, Gtk::POS_RIGHT, 1,
                         1);
    ...
    Gtk::Paned *viewpaned =
        Gtk::manage(new Gtk::Paned(Gtk::ORIENTATION_VERTICAL));
    fPanel = filePanel;
    ...
    info->signal_toggled().connect(
        sigc::mem_fun(*this, &EditorPanel::info_toggled));
    ...
    saveimgas->signal_button_release_event().connect_notify(
        sigc::mem_fun(*this, &EditorPanel::saveAsPressed));
    queueimg->signal_button_release_event().connect_notify(
        sigc::mem_fun(*this, &EditorPanel::queueImgPressed));
    sendtogimp->signal_button_release_event().connect_notify(
        sigc::mem_fun(*this, &EditorPanel::sendToGimpPressed));
...
void EditorPanel::open(Thumbnail *tmb, rtengine::InitialImage *isrc)
{
    ...
    //    iarea->fitZoom ();   // tell to the editorPanel that the next image
    //    has to be fitted to the screen
    ...
    // paramfile file
    std::unique_ptr<ProcParams> ldprof(openThm->createProcParamsForUpdate(
        true, false)); // will be freed by initProfile
    ...
    // When passing a photo as an argument to the RawTherapee executable, the
    // user wants this auto-loaded photo's thumbnail to be selected and visible
    // in the Filmstrip.
    syncFileBrowser();

    if (options.sidecar_autosave_interval > 0) {
        autosave_conn_ = Glib::signal_timeout().connect(
            sigc::mem_fun(*this, &EditorPanel::autosave),
            options.sidecar_autosave_interval * 60000);
    }
}
...
void EditorPanel::close()
{
    if (ipc) {
        ...
        if (iareapanel) {
            if (iareapanel->imageArea->mainCropWindow) {
                iareapanel->imageArea->mainCropWindow->cropHandler.newImage(
                    nullptr, false);
            }
...
void EditorPanel::saveProfile()
{
    ...
    if (Glib::file_test(fname, Glib::FILE_TEST_EXISTS)) {
        ...
        // Will call updateCache, which will update both the cached and sidecar
        // files if necessary
...
void EditorPanel::error(const Glib::ustring &descr) { parent->error(descr); }
...
void EditorPanel::displayError(const Glib::ustring &title,
...
void EditorPanel::refreshProcessingState(bool inProcessingP)
{
    ...
    if (inProcessingP) {
        ...
        // Maybe accessing "parent", which is a Gtk object, can justify to get
        // the Gtk lock...
        if (!firstProcessingDone &&
            static_cast<RTWindow *>(parent)->getIsFullscreen()) {
            parent->fullscreen();
        }
...
void EditorPanel::info_toggled()
{
    ...
    if (idata && idata->hasExif()) {
        ...
        expcomp = Glib::ustring(idata->expcompToString(
            idata->getExpComp(), true)); // maskZeroexpcomp
...
/*
 * WARNING: Take care of the simpleEditor value when adding or modifying
 * shortcut keys, since handleShortcutKey is now also triggered in simple editor
 * mode
 */
bool EditorPanel::handleShortcutKey(GdkEventKey *event)
{
    ...
    case GDK_KEY_L:
        if (tbTopPanel_1) {
            tbTopPanel_1->set_active(
                !tbTopPanel_1->get_active()); // toggle top panel
        }
    ...
    case GDK_KEY_l:
        if (alt) { // toggle left and right panels
            ...
            hidehp->set_active(
                !hidehp->get_active()); // toggle History (left panel)
        }
        return true;

        break;

    case GDK_KEY_m: // Maximize preview panel: hide top AND right AND history
                    // panels
        if (!ctrl && !alt) {
            toggleSidePanels();
            return true;
    ...
    if (!alt) {
        if (!ctrl) {
            ...
            /*
            #ifndef __WIN32__
                            case GDK_KEY_9: // toggle background color of the
            preview iareapanel->imageArea->previewModePanel->togglebackColor();
                                return true;
            #endif
            */
                                 ...
                                 rtengine::procparams::ProcParams &pparams)
{
    ...
    if (img) {
        ...
        if (sf.format == "tif") {
            ...
            ld->startFunc(
                sigc::slot0<int>(do_save),
                sigc::bind(sigc::mem_fun(*this, &EditorPanel::idle_imageSaved),
                           ld, img, fname, sf, pparams));
        }
    } else {
        Glib::ustring msg_ = Glib::ustring("<b>") + fname +
    ...
    const rtengine::procparams::PartialProfile *export_profile)
{
    ...
    isrc->getImageSource()->getFullSize(
        fullW, fullH,
        pparams.coarse.rotate == 90 || pparams.coarse.rotate == 270 ? TR_R90
                                                                    : TR_NONE);
    int prevh = BatchQueue::calcMaxThumbnailHeight();
    int prevw = int((size_t)fullW * (size_t)prevh / (size_t)fullH);
    return new BatchQueueEntry(job, pparams, openThm->getFileName(), prevw,
                               prevh, openThm);
}
...
void EditorPanel::do_save_image(bool fast_export)
{
    ...
    if (Glib::file_test(options.lastSaveAsPath, Glib::FILE_TEST_IS_DIR)) {
        ...
        saveAsDialog =
            new SaveAsDialog(PlacesBrowser::userPicturesDir(), toplevel);
    }
    ...
    do {
        ...
        saveAsDialog->get_size(options.saveAsDialogWidth,
                               options.saveAsDialogHeight);
        ...
        if (saveAsDialog->getImmediately()) {
            ...
            Glib::ustring dstfname =
                Glib::path_get_basename(removeExtension(fnameOut));
                ...
                for (int tries = 0; tries < 100; tries++) {
                    if (tries == 0) {
                        fnameTemp = Glib::ustring::compose(
                            "%1.%2", Glib::build_filename(dstdir, dstfname),
                            dstext);
                    } else {
                        fnameTemp = Glib::ustring::compose(
                            "%1-%2.%3", Glib::build_filename(dstdir, dstfname),
                            tries, dstext);
                    }
            ...
            BatchQueueEntry *bqe = createBatchQueueEntry(
                fast_export, false, saveAsDialog->getExportProfile());
                                  ...
                                  const SaveFormat &sf)
{
    ...
    if (gimpPlugin) {
        ...
        err = img->saveAsTIFF(filename, sf.tiffBits, sf.tiffFloat,
                              sf.tiffUncompressed);
    } else if (sf.format == "png") {
    ...
    ProgressConnector<rtengine::IImagefloat *> *pc, Glib::ustring fname)
{
    ...
    if (img) {
        ...
        Glib::ustring shortname =
            removeExtension(Glib::path_get_basename(fname));
        ...
        Glib::ustring fileName =
            Glib::ustring::compose("%1.%2", fname, sf.format);

        // TODO: Just list all file with a suitable name instead of brute
        // force...
        int tries = 1;
        ...
        while (Glib::file_test(fileName, Glib::FILE_TEST_EXISTS) &&
               tries < 1000) {
            fileName =
                Glib::ustring::compose("%1-%2.%3", fname, tries, sf.format);
            tries++;
        }
        ...
        Glib::ustring msg_ =
            Glib::ustring("<b> Error during image processing\n</b>");
        Gtk::MessageDialog msgd(*parent, msg_, true, Gtk::MESSAGE_ERROR,
                                Gtk::BUTTONS_OK, true);
        msgd.run();
        saveimgas->set_sensitive(true);
                                  ...
                                  Glib::ustring filename)
{
    ...
    if (!errore) {
        ...
        if (!success) {
            Gtk::MessageDialog msgd(*parent, M("MAIN_MSG_CANNOTSTARTEDITOR"),
...
void EditorPanel::beforeAfterToggled()
{
    ...
    if (beforeAfter->get_active()) {
        ...
        beforeLabel->set_markup(Glib::ustring("<b>") + M("GENERAL_BEFORE") +
                                "</b>");
        ...
        history->getBeforeAfterLock()
            ? tbBeforeLock->set_image(*iBeforeLockON)
        ...
        afterLabel->set_markup(Glib::ustring("<b>") + M("GENERAL_AFTER") +
                               "</b>");
...

        {
            ...
            cw->setPosition(0, 0);
            cw->enable(false);
            cw->cropHandler.cropParams =
                iareapanel->imageArea->mainCropWindow->cropHandler.cropParams;
    ...
    const array2D<int> &waveformLuma)
{
    if (histogramPanel) {
        histogramPanel->histogramChanged(
            histRed, histGreen, histBlue, histLuma, histChroma, histRedRaw,
    ...
    tpc->updateCurveBackgroundHistogram(histToneCurve, histLCurve, histCCurve,
...
void EditorPanel::toggleSidePanelsZoomFit()
{
    ...
    // fit image preview
    // !!! TODO this does not want to work... seems to have an effect on a
    // subsequent key press iarea->imageArea->zoomPanel->zoomFitClicked();
}
...
void EditorPanel::defaultMonitorProfileChanged(
    const Glib::ustring &profile_name, bool auto_monitor_profile)
{
    colorMgmtToolBar->defaultMonitorProfileChanged(profile_name,
                                                   auto_monitor_profile);
}
...
void EditorPanel::sizeChanged(int w, int h, int ow, int oh)
{
    if (ipc) {
        idle_register.add([this]() -> bool {
            if (ipc) {
                info_toggled();
                navigator->setInvalid(ipc->getFullWidth(),
                                      ipc->getFullHeight());
            }
            return false;
        });
