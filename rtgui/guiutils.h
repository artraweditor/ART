...
// TODO: The documentation says gdk_threads_enter and gdk_threads_leave should
// be replaced by g_main_context_invoke(), g_idle_add() and related functions,
// but this will require more extensive changes. We silence those warnings until
// then so that we notice the others.
...
public:
    explicit ConnectionBlocker(Gtk::Widget *associatedWidget,
        ...
        : connection(associatedWidget ? &connection : nullptr),
          wasBlocked(false)
    {
        if (this->connection) {
    ...
    explicit ConnectionBlocker(sigc::connection &connection)
...
/**
 * @brief Glue box to control visibility of the MyExpender's content ; also
 * handle the frame around it
 */
 ...
 * "enabled/disabled" property that display a different arrow depending on this
 * boolean value.
 *
 * Warning: once you've instantiated this class with a text label or a widget
 * label, you won't be able to revert to the other solution.
 */
...
private:
    ...
    Gtk::EventBox
        *titleEvBox; /// EventBox of the title, to get a connector from it
    Gtk::HBox *headerHBox;
    bool flushEvent; /// Flag to control the weird event mechanism of Gtk
                     /// (please prove me wrong!)
    ExpanderBox
        *expBox; /// Frame that includes the child and control its visibility
...
protected:
    Gtk::Container
        *child; /// Gtk::Contained to display below the expander's title
    Gtk::Widget *headerWidget; /// Widget to display in the header, next to the
                               /// arrow image ; can be NULL if the "string"
                               /// version of the ctor has been used
    RTImage *statusImage;      /// Image to display the opened/closed status (if
                               /// useEnabled is false) of the enabled/disabled
                               /// status (if useEnabled is true)
    Gtk::Label
        *label; /// Text to display in the header, next to the arrow image ; can
                /// be NULL if the "widget" version of the ctor has been used
    bool useEnabled; /// Set whether to handle an enabled/disabled feature and
...
public:
     ...
     * @param useEnabled Set whether to handle an enabled/disabled toggle button
     * and display the appropriate image
     * @param titleLabel A string to display in the header. Warning: you won't
     * be able to switch to a widget label.
     */
     ...
     * @param useEnabled Set whether to handle an enabled/disabled toggle button
     * and display the appropriate image
     * @param titleWidget A widget to display in the header. Warning: you won't
     * be able to switch to a string label.
     */
    ...
    /// Set a new label string. If it has been instantiated with a Gtk::Widget,
    ...
    bool getEnabled();
    /// If not inconsistent, set the enabled button to true or false and emit
    /// the message if the state is different If inconsistent, set the internal
    /// value to true or false, but do not update the image and do not emit the
    /// message
    ...
    /// Warning: do not manually Show/Hide the widget, because this parameter is
    /// handled by the click on the Expander's title
...
/**
 * @brief subclass of Gtk::ScrolledWindow in order to handle the large toolbars
...
public:
    ...
    void setPreferredWidth(int minimum_width, int natural_width);
    void connect(const sigc::connection &connection)
    {
...
/**
 * @brief A helper method to connect the current folder property of a file
 * chooser to an arbitrary variable.
 */
...
/**
 * @brief Handle the switch between text and image to be displayed in the HBox
 * (to be used in a button/toolpanel)
 */
class TextOrIcon: public Gtk::HBox {

public:
    TextOrIcon(const Glib::ustring &filename, const Glib::ustring &labelTx,
...
public:
    ...
    GradientMilestone(double _p = 0., double _r = 0., double _g = 0.,
                      double _b = 0., double _a = 0.)
    {
        position = _p;
        r = _r;
        g = _g;
...
/**
 * @brief Handle back buffers as automatically as possible, and suitable to be
 * used with Glib::RefPtr
 */
...
public:
    ...
    bool setDrawRectangle(Cairo::Format format, Gdk::Rectangle &rectangle,
                          bool updateBackBufferSize = true);
    bool setDrawRectangle(Cairo::Format format, int newX, int newY, int newW,
                          int newH, bool updateBackBufferSize = true);
    // set the destination drawing location, do not modify other parameters like
    // size and offset. Use setDrawRectangle to set all parameters at the same
    // time
    ...
    void copySurface(BackBuffer *destBackBuffer,
                     Gdk::Rectangle *rectangle = nullptr);
    void copySurface(Cairo::RefPtr<Cairo::ImageSurface> destSurface,
                     Gdk::Rectangle *rectangle = nullptr);
    void copySurface(Cairo::RefPtr<Cairo::Context> crDest,
    ...
    // you have to check if the surface is created thanks to surfaceCreated
    // before starting to draw on it
    ...
    int getWidth()
    {
        return surface ? surface->get_width()
                       : 0; // sending back the allocated width
    }
    int getHeight()
    {
        return surface ? surface->get_height()
                       : 0; // sending back the allocated height
    }
};

inline void setActiveTextOrIndex(Gtk::ComboBoxText &comboBox,
                                 const Glib::ustring &text, int index)
{
    bool valueSet = false;
    if (!text.empty()) {
