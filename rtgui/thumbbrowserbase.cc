...
ThumbBrowserBase::ThumbBrowserBase()
    : location(THLOC_FILEBROWSER), inspector(nullptr), isInspectorActive(false),
      ...
      previewHeight(options.thumbSize), numOfCols(1),
      arrangement(TB_Horizontal), use_hscroll_(true), use_vscroll_(true)
{
    inW = -1;
    inH = -1;

    setExpandAlignProperties(&internal, true, true, Gtk::ALIGN_FILL,
                             Gtk::ALIGN_FILL);
    setExpandAlignProperties(&hscroll, true, false, Gtk::ALIGN_FILL,
                             Gtk::ALIGN_CENTER);
    setExpandAlignProperties(&vscroll, false, true, Gtk::ALIGN_CENTER,
                             Gtk::ALIGN_FILL);
...
void ThumbBrowserBase::scrollChanged()
{
    ...
    // internal.setPosition ((int)(hscroll.get_value()),
    // (int)(vscroll.get_value()));
...
void ThumbBrowserBase::scroll(int direction, double deltaX, double deltaY)
{
    ...
    if (direction == GDK_SCROLL_SMOOTH && delta == 0.0) {
        // sometimes this case happens. To avoid scrolling the wrong direction
        // in this case, we just do nothing This is probably no longer necessary
        // now that coef is no longer quantized to +/-1.0 but why waste CPU
        // cycles?
        return;
    }
    // GDK_SCROLL_SMOOTH can come in as many events with small deltas, don't
    // quantize these to +/-1.0 so trackpads work well
    double coef;
...
{
    for (ThumbIterator thumb = selected.begin(); thumb != selected.end();
         ++thumb)
...
inline void removeFromSelection(const ThumbIterator &iterator,
                                ThumbVector &selected)
{
...
void ThumbBrowserBase::selectRange(ThumbBrowserEntryBase *clicked,
                                   bool additional)
{
    if (!anchor) {
        anchor = clicked;
...
{
    const ThumbIterator iterator =
        std::find(selected.begin(), selected.end(), clicked);
...
static void scrollToEntry(double &h, double &v, int iw, int ih,
            ...
            for (; curr >= fd.begin(); --curr) {
                if (!(*curr)->filtered) {
                    if (distance-- == 0) {
                        ...
                        // make sure the newly selected thumbnail is visible and
                        // make it current
                        scrollToEntry(h, v, internal.get_width(),
                                      internal.get_height(), *curr);
                        lastClicked = *curr;
...

    {
        ...
        if (!fd.empty()) {
            ...
            if (selected.empty()) {
                ...
                std::vector<ThumbBrowserEntryBase *>::iterator back =
                    std::find(fd.begin(), fd.end(),
                              lastEntry ? lastEntry : selected.back());
...

    {
        ...
        if (!fd.empty()) {
            ...
            if (selected.empty()) {
                ...
                std::vector<ThumbBrowserEntryBase *>::iterator front =
                    std::find(fd.begin(), fd.end(),
                              lastEntry ? lastEntry : selected.front());
...
void ThumbBrowserBase::configScrollBars()
{
    ...
    if (inW > 0 && inH > 0) {
        ...
        if (arrangement == TB_Horizontal) {
            ...
            ha->set_upper(inW);
            ha->set_lower(0);
            ha->set_step_increment(!fd.empty() ? fd[0]->getEffectiveWidth()
                                               : 0);
...
void ThumbBrowserBase::arrangeFiles(bool checkfilter)
{
    ...
    // We could lock it one more time, there's no harm excepted (negligible)
    // speed penalty
    // GThreadLock lock;
    ...
    if (arrangement == TB_Horizontal) {
        ...
        resizeThumbnailArea(currx, !fd.empty() ? fd[0]->getEffectiveHeight()
                                               : rowHeight);
    } else {
        ...
        for (; numOfCols > 0; --numOfCols) {
            ...
            // if not wider than the space available, arrange it and we are
            // ready
...
bool ThumbBrowserBase::Internal::on_configure_event(
...
{
    // Gtk signals automatically acquire the GUI (i.e. this method is enclosed
    // by gdk_thread_enter and gdk_thread_leave)
...
ThumbBrowserBase::Internal::Internal()
    : // ofsX(0), ofsY(0),
...
bool ThumbBrowserBase::Internal::on_button_press_event(GdkEventButton *event)
{
    ...
    parent->buttonPressed((int)event->x, (int)event->y, event->button,
                          event->type, event->state, 0, 0, get_width(),
                          get_height());
...
void ThumbBrowserBase::buttonPressed(int x, int y, int button,
                                     GdkEventType type, int state, int clx,
                                     int cly, int clw, int clh)
{
...

    {
        ...
        for (size_t i = 0; i < fd.size(); i++)
            if (fd[i]->drawable) {
                if (fd[i]->inside(x, y) &&
                    fd[i]->insideWindow(clx, cly, clw, clh)) {
                    fileDescr = fd[i];
                }
...
bool ThumbBrowserBase::Internal::on_draw(
...

    {
        ...
        for (size_t i = 0; i < parent->fd.size() && !dirty;
            ...
            if (!parent->fd[i]->drawable ||
                !parent->fd[i]->insideWindow(0, 0, w, h)) {
...
bool ThumbBrowserBase::Internal::on_button_release_event(GdkEventButton *event)
{
    ...
    for (size_t i = 0; i < parent->fd.size(); i++)
        if (parent->fd[i]->drawable &&
            ...
            MYREADERLOCK_RELEASE(l);
            // This will require a Writer access...
            tbe->releaseNotify(event->button, event->type, event->state,
...
void ThumbBrowserBase::zoomChanged(bool zoomIn)
{
    ...
    if (zoomIn)
        for (size_t i = 0; i < options.thumbnailZoomRatios.size(); i++) {
            newHeight =
                (int)(options.thumbnailZoomRatios[i] * getMaxThumbnailHeight());
...
void ThumbBrowserBase::refreshEditedState(const std::set<Glib::ustring> &efiles)
{
    ...
    editedFiles = efiles;
    {
        ...
        for (size_t i = 0; i < fd.size(); i++) {
            fd[i]->framed =
                editedFiles.find(fd[i]->filename) != editedFiles.end();
        }
    }
...
void ThumbBrowserBase::enableTabMode(bool enable)
{
    ...
    arrangement = enable ? ThumbBrowserBase::TB_Horizontal
                         : ThumbBrowserBase::TB_Vertical;

    if ((!options.sameThumbSize &&
         (options.thumbSizeTab != options.thumbSize)) ||
    ...
    // Tab mode is horizontal, file browser is vertical
    {
        ...
        if (!selected.empty()) {
            if (enable) {
                ...
                MYREADERLOCK_RELEASE(l);
                hscroll.set_value(
                    std::min(h, hscroll.get_adjustment()->get_upper()));
            } else {
                double v = selected[0]->getStartY();
                MYREADERLOCK_RELEASE(l);
                vscroll.set_value(
                    std::min(v, vscroll.get_adjustment()->get_upper()));
            }
        }
    }
}
...
void ThumbBrowserBase::redrawEntryNeeded(ThumbBrowserEntryBase *entry)
{
    ...
    if (entry->insideWindow(0, 0, internal.get_width(),
                            internal.get_height())) {
