                                       ...
                                       bool addHidden)
{
    ...
    try {
        ...
        if (options.rtSettings.verbose) {
            std::cerr << "Failed to list subdirectories of \""
                      << dir->get_parse_name() << "\": " << exception.what()
                      << std::endl;
        }
    }
...
DirBrowser::DirBrowser()
    : dirTreeModel(), dtColumns(), tvc(M("DIRBROWSER_FOLDERS")),
      selected_dir_(""), expandSuccess(false)
#ifdef WIN32
      ,
      volumes(0)
#endif
{
    ...
    scrolledwindow4->property_window_placement().set_value(
        Gtk::CORNER_TOP_LEFT);
...
void DirBrowser::fillDirTree()
{
    ...
    tvc.set_cell_data_func(
        crt, sigc::mem_fun(*this, &DirBrowser::on_cell_data_name));
    ...
    dirTreeModel->set_sort_column(dtColumns.filename,
    ...
    dirtree->signal_menu_item_activated().connect(
        sigc::mem_fun(*this, &DirBrowser::menu_item_activated));
    dirTreeModel->signal_sort_column_changed().connect(
        sigc::mem_fun(*this, &DirBrowser::on_sort_column_changed));
}

void DirBrowser::on_cell_data_name(Gtk::CellRenderer *renderer,
        ...
        for (int i = 0; i < 32; i++)
            if (((volumes >> i) & 1) &&
                ...
                for (Gtk::TreeModel::iterator iter =
                         dirTreeModel->children().begin();
                     iter != dirTreeModel->children().end(); ++iter)
                    if (iter->get_value(dtColumns.filename).c_str()[0] - 'A' ==
                        i) {
                              ...
                              const Gtk::TreeModel::Path &path)
{
    ...
    // We will disable model's sorting because it decreases speed of inserting
    // new items in list tree dramatically. Therefore will do: 1) Disable
    // sorting in model 2) Manually sort data in the order determined by the
    // options 3) Enable sorting in model again for UI (sorting by click on
    // header)
    int prevSortColumn;
    ...
    if (subDirs.empty()) {
        ...
        for (auto it = forErase.begin(), end = forErase.end(); it != end;
             ++it) {
    ...
    monitor->signal_changed().connect(
        sigc::bind(sigc::mem_fun(*this, &DirBrowser::file_changed), iter,
                   dir->get_parse_name()));
}
...
void DirBrowser::updateDir(const Gtk::TreeModel::iterator &iter)
{
    ...
    while (change) {
        ...
        for (Gtk::TreeModel::iterator it = iter->children().begin();
            ...
            if (!Glib::file_test(it->get_value(dtColumns.dirname),
                               ...
                               Gtk::TreeViewColumn *column)
{
    ...
    Glib::ustring dname =
        dirTreeModel->get_iter(path)->get_value(dtColumns.dirname);
...
{
    if (!dirtree->get_selection()->count_selected_rows())
        return;
                      ...
                      const Glib::ustring &fileName)
{
    if (art::session::check(dirname)) {
        ...
        // WARNING & TODO: One should test here if the directory/file has R/W
        // access permission to avoid crash
                              ...
                              const Glib::ustring &dirName)
{
    ...
    if (!file || !Glib::file_test(dirName, Glib::FILE_TEST_IS_DIR) ||
