    ...
    : CurveEditorSubGroup(curveDir)
{
    ...
    Gtk::Grid *CPointsbbox = Gtk::manage(
    ...
    initButton(*editCPoints, Glib::ustring("crosshair-node-curve.svg"),
               Gtk::ALIGN_START, false, "EDIT_PIPETTE_TOOLTIP");
    editPointCPoints = Gtk::manage(new Gtk::ToggleButton());
    initButton(*editPointCPoints, Glib::ustring("edit-point.svg"),
    ...
    initButton(*pasteCPoints, Glib::ustring("paste.svg"), Gtk::ALIGN_END,
               false);
    loadCPoints = Gtk::manage(new Gtk::Button());
    initButton(*loadCPoints, Glib::ustring("folder-open.svg"), Gtk::ALIGN_END,
               false);
...

    {
        ...
        axis.at(2).setValues(M("CURVEEDITOR_AXIS_LEFT_TAN"), 5, 0.01, 0.1, 0.,
                             1.);
        axis.at(3).setValues(M("CURVEEDITOR_AXIS_RIGHT_TAN"), 5, 0.01, 0.1, 0.,
                             1.);
        CPointsCoordAdjuster =
            Gtk::manage(new CoordinateAdjuster(CPointsCurve, this, axis));
        CPointsCoordAdjuster->get_style_context()->add_class(
            "curve-spinbuttonbox");
    }
    ...
    if (options.curvebboxpos == 0) {
        CPointsCurveGrid->attach_next_to(*CPointsbbox, *CPointsCurveBox,
                                         Gtk::POS_TOP, 1, 1);
    ...
    editPointCPointsConn =
        editPointCPoints->signal_toggled().connect(sigc::bind(
            sigc::mem_fun(*this, &FlatCurveEditorSubGroup::editPointToggled),
            editPointCPoints));
    editCPointsConn = editCPoints->signal_toggled().connect(
        sigc::bind(sigc::mem_fun(*this, &FlatCurveEditorSubGroup::editToggled),
                   editCPoints));
...
FlatCurveEditor *FlatCurveEditorSubGroup::addCurve(Glib::ustring curveLabel,
                                                   bool isPeriodic)
{
    FlatCurveEditor *newCE =
        new FlatCurveEditor(curveLabel, parent, this, isPeriodic);
...
void FlatCurveEditorSubGroup::showCoordinateAdjuster(
...
void FlatCurveEditorSubGroup::refresh(CurveEditor *curveToRefresh)
{
    if (curveToRefresh != nullptr &&
        curveToRefresh ==
            static_cast<FlatCurveEditor *>(parent->displayedCurve)) {
...
void FlatCurveEditorSubGroup::pipetteDrag(EditDataProvider *provider,
                                          int modifierKey)
{
    CurveEditor *curveEditor =
        static_cast<FlatCurveEditor *>(parent->displayedCurve);
...
void FlatCurveEditorSubGroup::restoreDisplayedHistogram()
{
    if (parent->displayedCurve) {
        ...
        CPointsCurve->updateBackgroundHistogram(
            parent->displayedCurve->histogram);
    }
}
...
bool FlatCurveEditorSubGroup::curveReset(CurveEditor *ce)
{
    ...
    case (FCT_MinMaxCPoints): // = Control cage
        CPointsCurve->reset(fce->controlPointsResetCurve,
                            fce->getIdentityValue());
        return true;
        break;
...
: NULL); customCurve->updateBackgroundHistogram (fce->bgHistValid ?
fce->histogram : NULL); NURBSCurve->updateBackgroundHistogram (fce->bgHistValid
? fce->histogram : NULL);
    }
}*/
