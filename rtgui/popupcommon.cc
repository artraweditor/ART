                           ...
                           const Glib::ustring &label)
{
    ...
    MyImageMenuItem *newItem =
        Gtk::manage(new MyImageMenuItem(label, fileName));
    ...
    if (images.size() == 1) {
        ...
        arrowImage->set_from_icon_name("pan-down-symbolic",
                                       Gtk::ICON_SIZE_BUTTON);
        setExpandAlignProperties(arrowButton, false, false, Gtk::ALIGN_CENTER,
                                 Gtk::ALIGN_FILL);
        arrowButton->add(*arrowImage); // menuSymbol);
        buttonGroup->attach_next_to(*arrowButton, *button, Gtk::POS_RIGHT, 1,
                                    1);
    ...
    newItem->signal_activate().connect(sigc::bind(
        sigc::mem_fun(*this, &PopUpCommon::entrySelected), images.size() - 1));
    menu->append(*newItem);
...
bool PopUpCommon::setSelected(int entryNum)
{
    ...
    if (entryNum < 0 || entryNum > ((int)images.size() - 1) ||
        ...
        // Maybe we could do something better than loading the image file each
