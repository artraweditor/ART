                        ...
                        true)
{
    ...
    curveEditorG = new CurveEditorGroup(options.lastRgbCurvesDir,
                                        ""); // M("TP_RGBCURVES_CHANNEL"));
...
void RGBCurves::renderCurveBackground(int caller_id,
    ...
    for (int i = 0; i < 3; ++i) {
        if (i != caller_id) {
            ...
            if (ci != mine) {
                ...
                for (int i = 1; i < w; ++i) {
                    cr->line_to(double(i) + x,
                                curve.getVal(double(i) / double(w)) * -h + y);
                }
