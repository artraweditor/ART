      ...
      isClosed(true)
{
    ...
    signal_key_press_event().connect(
        sigc::mem_fun(*this, &EditWindow::keyPressed));
    signal_key_press_event().connect(
        sigc::mem_fun(*this, &EditWindow::keyPressedBefore), false);
    signal_key_release_event().connect(
        sigc::mem_fun(*this, &EditWindow::keyReleased));
    signal_scroll_event().connect(
        sigc::mem_fun(*this, &EditWindow::scrollPressed), false);
 ...
 *  Someday, the EditorWindow might save its own position and state, so it'll
have to be uncommented bool
EditWindow::on_window_state_event(GdkEventWindowState* event)
{
    if (event->changed_mask & GDK_WINDOW_STATE_MAXIMIZED) {
        options.windowMaximized = event->new_window_state &
...
void EditWindow::toFront()
{
    ...
    // we need to present the secondary window. If we don't, it will stay in
    // background when opening 2nd, 3rd... editor, which is annoying It will
    // also deiconify the window
    present();
}
...
void EditWindow::writeOptions()
{
    ...
    if (is_visible()) {
        if (isMultiDisplayEnabled()) {
            options.meowMonitor =
                get_screen()->get_monitor_at_window(get_window());
        }
