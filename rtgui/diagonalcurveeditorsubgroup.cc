    ...
    : CurveEditorSubGroup(curveDir)
{
    ...
    Gtk::PositionType sideStart =
        options.curvebboxpos == 0 || options.curvebboxpos == 2 ? Gtk::POS_LEFT
                                                               : Gtk::POS_TOP;
    Gtk::PositionType sideEnd =
        options.curvebboxpos == 0 || options.curvebboxpos == 2
            ? Gtk::POS_RIGHT
            : Gtk::POS_BOTTOM;
    ...
    Gtk::Grid *custombbox = Gtk::manage(
        new Gtk::Grid()); // curvebboxpos 0=above, 1=right, 2=below, 3=left
    ...
    initButton(*editPointCustom, Glib::ustring("edit-point.svg"),
               Gtk::ALIGN_START, false, "CURVEEDITOR_EDITPOINT_HINT");
    editCustom = Gtk::manage(new Gtk::ToggleButton());
    initButton(*editCustom, Glib::ustring("crosshair-node-curve.svg"),
               Gtk::ALIGN_START, false, "EDIT_PIPETTE_TOOLTIP");
    editCustom->hide();
    ...
    initButton(*loadCustom, Glib::ustring("folder-open.svg"), Gtk::ALIGN_END,
               false);
    ...
    customCoordAdjuster =
        Gtk::manage(new CoordinateAdjuster(customCurve, this));
    ...
    if (options.curvebboxpos == 0) {
        customCurveGrid->attach_next_to(*custombbox, *customCurveBox,
                                        Gtk::POS_TOP, 1, 1);
    ...
    editPointCustomConn = editPointCustom->signal_toggled().connect(sigc::bind(
        sigc::mem_fun(*this, &DiagonalCurveEditorSubGroup::editPointToggled),
        editPointCustom));
    editCustomConn = editCustom->signal_toggled().connect(sigc::bind(
        sigc::mem_fun(*this, &DiagonalCurveEditorSubGroup::editToggled),
        editCustom));
    ...
    initButton(*editPointNURBS, Glib::ustring("edit-point.svg"),
               Gtk::ALIGN_START, false, "CURVEEDITOR_EDITPOINT_HINT");
    editNURBS = Gtk::manage(new Gtk::ToggleButton());
    initButton(*editNURBS, Glib::ustring("crosshair-node-curve.svg"),
               Gtk::ALIGN_START, false, "EDIT_PIPETTE_TOOLTIP");
    editNURBS->hide();
    ...
    initButton(*loadNURBS, Glib::ustring("folder-open.svg"), Gtk::ALIGN_END,
               false);
    ...
    if (options.curvebboxpos == 0) {
        NURBSCurveGrid->attach_next_to(*NURBSbbox, *NURBSCurveBox, Gtk::POS_TOP,
                                       1, 1);
        NURBSCurveGrid->attach_next_to(*NURBSCoordAdjuster, *NURBSCurveBox,
                                       Gtk::POS_BOTTOM, 1, 1);
    } else if (options.curvebboxpos == 1) {
        NURBSCurveGrid->attach_next_to(*NURBSbbox, *NURBSCurveBox,
                                       Gtk::POS_RIGHT, 1, 1);
        NURBSCurveGrid->attach_next_to(*NURBSCoordAdjuster, *NURBSCurveBox,
                                       Gtk::POS_BOTTOM, 2, 1);
    } else if (options.curvebboxpos == 2) {
        NURBSCurveGrid->attach_next_to(*NURBSCoordAdjuster, *NURBSCurveBox,
                                       Gtk::POS_BOTTOM, 1, 1);
        NURBSCurveGrid->attach_next_to(*NURBSbbox, *NURBSCoordAdjuster,
                                       Gtk::POS_BOTTOM, 1, 1);
    } else if (options.curvebboxpos == 3) {
        NURBSCurveGrid->attach_next_to(*NURBSbbox, *NURBSCurveBox,
                                       Gtk::POS_LEFT, 1, 1);
        NURBSCurveGrid->attach_next_to(*NURBSCoordAdjuster, *NURBSbbox,
                                       Gtk::POS_BOTTOM, 2, 1);
    }
    ...
    copyNURBS->signal_clicked().connect(
        sigc::mem_fun(*this, &DiagonalCurveEditorSubGroup::copyPressed));
    editPointNURBSConn = editPointNURBS->signal_toggled().connect(sigc::bind(
        sigc::mem_fun(*this, &DiagonalCurveEditorSubGroup::editPointToggled),
        editPointNURBS));
    ...
    paramCurveBox->attach_next_to(*shcSelector, *paramCurve, Gtk::POS_BOTTOM, 1,
                                  1);

    editParam = Gtk::manage(new Gtk::ToggleButton());
    initButton(*editParam, Glib::ustring("crosshair-node-curve.svg"),
               Gtk::ALIGN_START, false, "EDIT_PIPETTE_TOOLTIP");
    editParam->hide();
    ...
    initButton(*loadParam, Glib::ustring("folder-open.svg"), Gtk::ALIGN_END,
               false);
    ...
    lights =
        Gtk::manage(new Adjuster(M("CURVEEDITOR_LIGHTS"), -100, 100, 1, 0));
    darks = Gtk::manage(new Adjuster(M("CURVEEDITOR_DARKS"), -100, 100, 1, 0));
    shadows =
        Gtk::manage(new Adjuster(M("CURVEEDITOR_SHADOWS"), -100, 100, 1, 0));
    ...
    // paramCurveSliderBox needed to set vspacing(4) between curve+shc and
    // sliders without vspacing between each slider
    ...
    if (options.curvebboxpos == 0) {
        paramCurveGrid->attach_next_to(*parambbox, *paramCurveBox, Gtk::POS_TOP,
                                       1, 1);
        paramCurveGrid->attach_next_to(*paramCurveSliderBox, *paramCurveBox,
                                       Gtk::POS_BOTTOM, 1, 1);
    } else if (options.curvebboxpos == 1) {
        paramCurveGrid->attach_next_to(*parambbox, *paramCurveBox,
                                       Gtk::POS_RIGHT, 1, 1);
        paramCurveGrid->attach_next_to(*paramCurveSliderBox, *paramCurveBox,
                                       Gtk::POS_BOTTOM, 2, 1);
    } else if (options.curvebboxpos == 2) {
        paramCurveGrid->attach_next_to(*parambbox, *paramCurveBox,
                                       Gtk::POS_BOTTOM, 1, 1);
        paramCurveGrid->attach_next_to(*paramCurveSliderBox, *parambbox,
                                       Gtk::POS_BOTTOM, 1, 1);
    } else if (options.curvebboxpos == 3) {
        paramCurveGrid->attach_next_to(*parambbox, *paramCurveBox,
                                       Gtk::POS_LEFT, 1, 1);
    ...
    customCurve->setCurveListener(
        parent); // Send the message directly to the parent
    ...
    evhighlights->signal_enter_notify_event().connect(sigc::bind(
        sigc::mem_fun(*this, &DiagonalCurveEditorSubGroup::adjusterEntered),
        4));
 ...
 */
DiagonalCurveEditor *
DiagonalCurveEditorSubGroup::addCurve(Glib::ustring curveLabel)
{
    DiagonalCurveEditor *newCE =
        new DiagonalCurveEditor(curveLabel, parent, this);
...
void DiagonalCurveEditorSubGroup::pipetteButton1Released(
...
void DiagonalCurveEditorSubGroup::pipetteDrag(EditDataProvider *provider,
                                              int modifierKey)
{
    CurveEditor *curveEditor =
        static_cast<DiagonalCurveEditor *>(parent->displayedCurve);
    ...
    case (DCT_Parametric):
        if (editedAdjuster) {
            int trimmedValue =
                editedAdjusterValue - (provider->deltaScreen.y / 2);
...
void DiagonalCurveEditorSubGroup::showCoordinateAdjuster(
...
/*
 * Force the resize of the curve editor, if the displayed one is the requested
 * one
 */
...
void DiagonalCurveEditorSubGroup::switchGUI()
{
    ...
    if (dCurve) {
        ...
        if (barColorProvider == nullptr && bgGradient.size() == 0) {
            ...
            // dCurve ave a ColorProvider or a background gradient defined, so
            // we create/update the object
            if (!leftBar) {
        ...
        if (barColorProvider == nullptr && bgGradient.size() == 0) {
            ...
            // dCurve has a ColorProvider or a background gradient defined, so
            // we create/update the object
            if (!bottomBar) {
        ...
        switch (auto tp =
                    (DiagonalCurveType)(dCurve->curveType->getSelected())) {
        case (DCT_Spline):
        case (DCT_CatmullRom):
            customCurve->setPoints(tp == DCT_Spline
                                       ? dCurve->customCurveEd
                                       : dCurve->catmullRomCurveEd);
            customCurve->setColorProvider(dCurve->getCurveColorProvider(),
                                          dCurve->getCurveCallerId());
            customCurve->setBackgroundProvider(dCurve->getBackgroundProvider(),
                                               dCurve->getBackgroundCallerId());
        ...
        case (DCT_Parametric): {
            ...
            paramCurve->setBackgroundProvider(dCurve->getBackgroundProvider(),
                                              dCurve->getBackgroundCallerId());
            ...
            shcSelector->setMargins(
                (int)(((leftBar ? (double)CBAR_WIDTH + 2. +
                                      (double)CBAR_MARGIN + RADIUS
                                : RADIUS) -
                       1.5) *
                      s),
                (int)((RADIUS - 1.5) * s));
        ...
        case (DCT_NURBS):
            ...
            NURBSCurve->setColorProvider(dCurve->getCurveColorProvider(),
                                         dCurve->getCurveCallerId());
            NURBSCurve->setBackgroundProvider(dCurve->getBackgroundProvider(),
                                              dCurve->getBackgroundCallerId());
...
void DiagonalCurveEditorSubGroup::editToggled(Gtk::ToggleButton *button)
{
    ...
    if (button->get_active()) {
        ...
        if (button != editCustom) {
            ...
            // will throw the event of curveChanged, which will now build the
        edit's buffer customCurve->notifyListener ();
        }
        if (button != editNURBS) {
        ...
        */
        ...
        editCustom->set_active(false);  editCustomConn.block(false); } if
        (button != editNURBS  ) { editNURBSConn.block(true);
        editNURBS->set_active(false);   editNURBSConn.block(false);  } if
        (button != editParam  ) { editParamConn.block(true);
        editParam->set_active(false);   editParamConn.block(false);  }
        */
    }
}

/*
 * Store the curves of the currently displayed type from the widgets to the
 * CurveEditor object
 */
    ...
    if (parent->displayedCurve) {
        ...
        case (DCT_Spline):
            storeCurveValues(parent->displayedCurve,
                             getCurveFromGUI(DCT_Spline));
            break;

        case (DCT_Parametric):
            storeCurveValues(parent->displayedCurve,
                             getCurveFromGUI(DCT_Parametric));
            break;

        case (DCT_NURBS):
            storeCurveValues(parent->displayedCurve,
                             getCurveFromGUI(DCT_NURBS));
            break;

        case (DCT_CatmullRom):
            storeCurveValues(parent->displayedCurve,
                             getCurveFromGUI(DCT_CatmullRom));
            break;
...
void DiagonalCurveEditorSubGroup::restoreDisplayedHistogram()
{
    if (parent->displayedCurve /*&& initslope==1*/) {
        paramCurve->updateBackgroundHistogram(
            parent->displayedCurve->histogram);
        customCurve->updateBackgroundHistogram(
            parent->displayedCurve->histogram);
        NURBSCurve->updateBackgroundHistogram(
            parent->displayedCurve->histogram);
    }
}
