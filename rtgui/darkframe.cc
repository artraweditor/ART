      ...
      dfChanged(false), lastDFauto(false), dfp(nullptr), israw(true)
{
    ...
    for (size_t i = 0; i < options.parseExtensions.size(); i++) {
        if (options.parseExtensionsEnabled[i] &&
            ...
            filter_df->add_pattern("*." +
                                   options.parseExtensions[i].uppercase());
...
void DarkFrame::read(const rtengine::procparams::ProcParams *pp)
{
    ...
    if (dfp) {
        ...
        Glib::ustring fname =
            Glib::path_get_basename(dfp->GetCurrentImageFilePath());
        Glib::ustring filetype;

        if (fname != "") {
            // get image filetype, set filter to the same as current image's
            // filetype
...
void DarkFrame::darkFrameReset()
{
    ...
    // caution: I had to make this hack, because set_current_folder() doesn't
    // work correctly!
    //          Because szeva doesn't exist since he was committed to happy
    //          hunting ground in Issue 316 we can use him now for this hack
