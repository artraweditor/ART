...
MyDiagonalCurve::MyDiagonalCurve()
    : MyCurve(), closest_point(0), clampedX(0.0), clampedY(0.0), deltaX(0.0),
      ...
      activeParam(-1), bghistvalid(false), bp_(nullptr), bp_id_(-1)
{
    ...
    signal_event().connect(
        sigc::mem_fun(*this, &MyDiagonalCurve::handleEvents));
...
void MyDiagonalCurve::setBackgroundProvider(CurveBackgroundProvider *bp,
                                            int caller_id)
{
    bp_ = bp;
    bp_id_ = caller_id;
...
void MyDiagonalCurve::draw(int handle)
{
    ...
    if (bghistvalid) {
        ...
        for (int i = 0; i < 256; i++) {
            double val =
                double(bghist[i]) * double(graphH - 2) / double(valMax);
            /*
            if (val>graphH-2)
                val = graphH-2;
            */
    ...
    for (int i = 0; i <= 10; i++) {
        ...
        cr->move_to(graphX - 0.5 - 0.5 * s,
                    graphY + 0.5 + 0.5 * s -
                        (graphH + 1. + 1. * s) * (double)i / 10.);
        cr->rel_line_to(graphW + 1. + 1. * s, 0.);
        // vertical lines
        cr->move_to(graphX - 0.5 - 0.5 * s +
                        (graphW + 1. + 1. * s) * (double)i / 10.,
                    graphY + 0.5 + 0.5 * s);
    ...
    if (bp_) {
        bp_->renderCurveBackground(bp_id_, style, cr, graphX, graphY, graphW,
                                   graphH);
    }
    ...
    if (curve.type == DCT_NURBS) {
        ...
        for (unsigned int i = 1; i < nbPoints; i++) {
            ...
            double x1 =
                graphX +
                graphW *
                    points[pos - 2]; // project (curve.at(i), 0, 1, graphW);
            double y1 =
                graphY -
                graphH *
                    points[pos - 1]; // project (curve.y.at(i)i], 0, 1, graphH);
            double x2 =
                graphX +
                graphW * points[pos]; // project (curve.at(i), 0, 1, graphW);
            double y2 =
                graphY -
                graphH *
                    points[pos + 1]; // project (curve.y.at(i), 0, 1, graphH);

            // set the color of the line when the point is snapped to the cage
            if (curve.x.size() == nbPoints && snapToElmt >= 1000 &&
                ((int(i) == (snapToElmt - 1000)) ||
                 (int(i) == (snapToElmt - 999)))) {
                cr->set_source_rgb(1.0, 0.0, 0.0);
            } else {
                cr->set_source_rgb(c.get_red(), c.get_green(), c.get_blue());
            }
        ...
        for (int i = 0; i < (int)curve.x.size(); ++i) {
            ...
            if (snapToElmt >= 1000) {
                ...
                if (i >= (pt - 1) && i <= (pt + 1)) {
                    ...
                    cr->set_source_rgb(c.get_red(), c.get_green(),
                                       c.get_blue());
                }
            } else {
...
bool MyDiagonalCurve::handleEvents(GdkEvent *event)
{
        ...
        if (curve.type != DCT_Parametric) {
            if (edited_point == -1) {
                if (event->button.button == 1) {
                    ...
                    getCursorPosition(
                        Gdk::EventType(event->type), event->motion.is_hint != 0,
    ...
    case GDK_MOTION_NOTIFY:
        ...
        if (curve.type == DCT_Linear || curve.type == DCT_Spline ||
            curve.type == DCT_NURBS || curve.type == DCT_CatmullRom) {
            ...
            if (grab_point == -1) {
                if (edited_point == -1) {
...

                    {
                        ...
                        if (cursorX < -RADIUS || cursorX > extendedGraphW ||
                            ...
                            // the cursor is close to an existing point
                            new_type = CSPlus; // Shown when hovering over node
                                               // snapping distance (not
                                               // necessarily over node).
                            lit_point = closest_point;
                        } else {
                            // the cursor is inside the graph but away from
                            // existing points
                    ...
                    if (distanceX <= minDistanceX) {
                        ...
                        // the cursor is outside the graph or inside the graph
                        // but away from existing points
                        lit_point = -1;
                    }
                ...
                // we memorize the previous position of the point, for
                ...
                if (ugpY >= topDeletionBound && grab_point != 0 &&
                    ...
                    if (snapTo && curve.x.at(grab_point) != -1.) {
                        if (grab_point > 0 &&
                            ...
                            double ratio = (curve.x.at(grab_point) - prevX) /
                                           (nextX - prevX);
...
CursorShape MyDiagonalCurve::motionNotify(CursorShape type, double minDistanceX,
...
bool MyDiagonalCurve::pipetteButton1Pressed(EditDataProvider *provider,
                                            int modifierKey)
{
    if (edited_point > 1) {
        return false;
    ...
    if (curve.type != DCT_Parametric) {
        ...
        if (distanceX > minDistanceX) {
            ...
            // the graph is refreshed only if a new point is created (snapped to
            // a pixel)
            if (lit_point >= 0) {
...
{
    if (edited_point > -1 || curve.type == DCT_Parametric || graphW < 0 ||
        graphH < 0) {
        return;
    }
...
void MyDiagonalCurve::getCursorPosition(Gdk::EventType evType, bool isHint,
                                        int evX, int evY,
                                        Gdk::ModifierType modifierKey)
{
    int tx, ty;
    ...
    if (curve.type != DCT_Parametric) {
        for (int i = 0; i < (int)curve.x.size(); i++) {
            ...
            if (currDistX < distanceX) {
                ...
                // there is more than 1 point for that X coordinate, we select
...
void MyDiagonalCurve::reset(const std::vector<double> &resetCurve,
