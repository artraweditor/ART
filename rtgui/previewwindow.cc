...
PreviewWindow::PreviewWindow()
    : previewHandler(nullptr), mainCropWin(nullptr), imageArea(nullptr),
      ...
      isMoving(false), needsUpdate(false), cursor_type(CSUndefined)
...
void PreviewWindow::updatePreviewImage()
{
    ...
    if (previewHandler) {
        Glib::RefPtr<Gdk::Pixbuf> resPixbuf =
            previewHandler->getRoughImage(W, H, zoom);
...
bool PreviewWindow::on_draw(const ::Cairo::RefPtr<Cairo::Context> &cr)
{
    ...
    if (mainCropWin && zoom > 0.0) {
        ...
        if (x > imgX || y > imgY || w < imgW || h < imgH) {
            ...
            cr->rectangle(rectX + 1. * s, rectY + 1. * s, rectW - 2. * s,
                          rectH - 2. * s);
            cr->stroke();

            // draw a "frame" line. Color of frame line can be set in
            // preferences
            cr->set_source_rgba(
                options.navGuideBrush[0], options.navGuideBrush[1],
...
bool PreviewWindow::on_button_press_event(GdkEventButton *event)
{
    ...
    if (x > imgX || y > imgY || w < imgW || h < imgH) {
        ...
        if (!isMoving) {
            ...
            mainCropWin->remoteMove((event->x - x - 0.5 * w) / zoom,
                                    (event->y - y - 0.5 * h) / zoom);
