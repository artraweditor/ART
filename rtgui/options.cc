...
void Options::updatePaths()
{
    ...
    if (Glib::path_is_absolute(profilePath)) {
        ...
        if (!checkDirPath(profilePath, "")) {
            ...
            if (!checkDirPath(
                    profilePath,
                    "")) { // had problems with mkdir_with_parents return value
                           // on OS X, just check dir again
                Glib::ustring msg =
                    Glib::ustring::compose("Creation of the user's processing "
                                           "profile directory \"%1\" failed!\n",
                                           profilePath);
                throw Error(msg);
            }
        }
        ...
        if (checkDirPath(profilePath,
                         "Error: the user's processing profile path doesn't "
            ...
            if (checkDirPath(
                    tmpPath,
                    "Error: the global's processing profile path doesn't point "
        ...
        if (checkDirPath(tmpPath, "Error: the user's processing profile path "
...
/** @brief Get the absolute path of the given filename or the "Neutral" special
 *value
 *
 *@param profName  path + filename of the procparam to look for. A filename
 *without path can be provided for backward compatibility. In this case, this
 *parameter will be updated with the new format.
 *@return Send back the absolute path of the given filename or "Neutral" if
 *"Neutral" has been set to profName. Implementor will have to test for this
 *particular value. If the absolute path is invalid (e.g. the file doesn't
...
Glib::ustring Options::findProfilePath(Glib::ustring &profName)
{
    ...
    if (p == "${U}") {
        ...
        p = getUserProfilePath();
        Glib::ustring fullPath =
            Glib::build_filename(p, profName + paramFileExtension);
...
void Options::setDefaults()
{
    ...
    version = "0.0.0.0"; // temporary value; will be correctly set in
                         // RTWindow::on_realize
    thumbSize = 160;
    thumbSizeTab = 160;
    thumbSizeQueue = 160;
    sameThumbSize = false; // preferring speed of switch between file browser
                           // and single editor tab
    ...
    lastSaveAsPath = "";
    overwriteOutputFile =
        false; // if TRUE, existing output JPGs/PNGs are overwritten, instead of
               // adding ..-1.jpg, -2.jpg etc.
    theme = "Default";
    ...
    saveParamsFile =
        true; // was false, but saving the procparams files next to the file
              // make more sense when reorganizing file tree than in a cache
    saveParamsCache = false; // there's no need to save the procparams files in
                             // a cache if saveParamsFile is true
    ...
    internalThumbIfUntouched =
        true; // if TRUE, only fast, internal preview images are taken if the
              // image is not edited yet
    showFileNames = true;
    ...
    maxInspectorBuffers =
        2; //  a rather conservative value for low specced systems...
    inspectorDelay = 0;
    ...
    if (sysRoot != NULL) {
        rtSettings.iccDirectory =
            Glib::ustring(sysRoot) +
            Glib::ustring("\\System32\\spool\\drivers\\color");
    } else {
        rtSettings.iccDirectory =
    ...
    // There is no reasonable default for curves. We can still suppose that they
    // will take place in a subdirectory of the user's own ProcParams presets,
    // i.e. in a subdirectory of the one pointed to by the "profile" field. The
    // following fields will then be initialized when "profile" will have its
    // final value, at the end of the "updatePaths" method.
    lastRgbCurvesDir = "";
    lastLabCurvesDir = "";
    lastPFCurvesDir = "";
    lastHsvCurvesDir = "";
    ...
    falseColorsMap = {{2, "#FFFFFF"},  {10, "#0000FF"},  {20, "#2290FF"},
                      {42, "#4B4B4B"}, {48, "#FF11FC"},  {52, "#7B7B7B"},
                      {58, "#00FF00"}, {78, "#ADADAD"},  {84, "#AEAE00"},
                      {94, "#FFFF00"}, {100, "#FF7F00"}, {108, "#FF0000"}};
...
void Options::readFromFile(Glib::ustring fname)
{
    ...
    try {
        if (keyFile.load_from_file(fname)) {
            ...
            if (keyFile.has_group("General")) {
                ...
                if (keyFile.has_key("General", "StartupDirectory")) {
                    if (keyFile.get_string("General", "StartupDirectory") ==
                        "home") {
                        startupDir = STARTUPDIR_HOME;
                    } else if (keyFile.get_string("General",
                                                  "StartupDirectory") ==
                               "current") {
                        startupDir = STARTUPDIR_CURRENT;
                    } else if (keyFile.get_string(
                                   "General", "StartupDirectory") == "last") {
                        startupDir = STARTUPDIR_LAST;
                    } else if (keyFile.get_string(
                                   "General", "StartupDirectory") == "custom") {
                ...
                if (keyFile.has_key("General", "Verbose")) {
                    try {
                        rtSettings.verbose =
                            keyFile.get_integer("General", "Verbose");
                    } catch (Glib::Error &e) {
                        rtSettings.verbose =
                            keyFile.get_boolean("General", "Verbose");
                    }
                }

                if (keyFile.has_key("General", "ErrorMessageDuration")) {
                    error_message_duration =
                        keyFile.get_integer("General", "ErrorMessageDuration");
                }

                if (keyFile.has_key("General", "MaxErrorMessages")) {
                    max_error_messages =
                        keyFile.get_integer("General", "MaxErrorMessages");
                }

                if (keyFile.has_key("General", "EditorKeyboardScrollStep")) {
                    editor_keyboard_scroll_step = keyFile.get_integer(
                        "General", "EditorKeyboardScrollStep");
                }

                if (keyFile.has_key("General",
                                    "AdjusterShortcutScrollWheelFactor")) {
                    adjuster_shortcut_scrollwheel_factor = keyFile.get_integer(
                        "General", "AdjusterShortcutScrollWheelFactor");
                }
            }
            ...
            if (keyFile.has_group("External Editor")) {
                ...
                if (keyFile.has_key("External Editor", "PhotoshopDir")) {
                    psDir =
                        keyFile.get_string("External Editor", "PhotoshopDir");
                }
                ...
                if (keyFile.has_key("External Editor", "CustomOutputDir")) {
                    editor_custom_out_dir = keyFile.get_string(
                        "External Editor", "CustomOutputDir");
                }

                if (keyFile.has_key("External Editor", "Float32")) {
                    editor_float32 =
                        keyFile.get_boolean("External Editor", "Float32");
                }

                if (keyFile.has_key("External Editor", "BypassOutputProfile")) {
                    editor_bypass_output_profile = keyFile.get_boolean(
                        "External Editor", "BypassOutputProfile");
                }
            }
            ...
            if (keyFile.has_group("Output")) {
                ...
                if (keyFile.has_key("Output", "PngBps")) {
                    saveFormat.pngBits =
                        keyFile.get_integer("Output", "PngBps");
                }

                if (keyFile.has_key("Output", "TiffBps")) {
                    saveFormat.tiffBits =
                        keyFile.get_integer("Output", "TiffBps");
                }

                if (keyFile.has_key("Output", "TiffFloat")) {
                    saveFormat.tiffFloat =
                        keyFile.get_boolean("Output", "TiffFloat");
                }
                ...
                if (keyFile.has_key("Output", "PathTemplate")) {
                    savePathTemplate =
                        keyFile.get_string("Output", "PathTemplate");
                }
                ...
                if (keyFile.has_key("Output", "ForceFormatOpts")) {
                    forceFormatOpts =
                        keyFile.get_boolean("Output", "ForceFormatOpts");
                }

                if (keyFile.has_key("Output", "SaveMethodNum")) {
                    saveMethodNum =
                        keyFile.get_integer("Output", "SaveMethodNum");
                }
                ...
                if (keyFile.has_key("Output", "LastSaveAsPath")) {
                    lastSaveAsPath =
                        keyFile.get_string("Output", "LastSaveAsPath");
                }
                ...
                if (keyFile.has_key("Output", "ProcParamsAutosaveInterval")) {
                    sidecar_autosave_interval = keyFile.get_integer(
                        "Output", "ProcParamsAutosaveInterval");
                }
            }
            ...
            if (keyFile.has_group("Profiles")) {
                ...
                if (keyFile.has_key("Profiles", "AppendMode")) {
                    profile_append_mode =
                        keyFile.get_boolean("Profiles", "AppendMode");
                }
                ...
                if (keyFile.has_key("Profiles", "EmbedParamsInMetadata")) {
                    params_out_embed = keyFile.get_boolean(
                        "Profiles", "EmbedParamsInMetadata");
                }

                if (keyFile.has_key("Profiles",
                                    "ParamsSidecarStripExtension")) {
                    params_sidecar_strip_extension = keyFile.get_boolean(
                        "Profiles", "ParamsSidecarStripExtension");
                }

                if (keyFile.has_key("Profiles", "CustomProfileBuilder")) {
                    CPBPath = keyFile.get_string(
                        "Profiles",
                        "CustomProfileBuilder"); // for backward compatibility
                                                 // only
                }
            ...
            if (keyFile.has_group("File Browser")) {
                if (keyFile.has_key("File Browser", "ThumbnailSize")) {
                    thumbSize =
                        keyFile.get_integer("File Browser", "ThumbnailSize");
                }
                ...
                if (keyFile.has_key("File Browser", "ThumbnailSizeQueue")) {
                    thumbSizeQueue = keyFile.get_integer("File Browser",
                                                         "ThumbnailSizeQueue");
                }
                ...
                if (keyFile.has_key("File Browser", "ThumbnailOrder")) {
                    thumbnailOrder = ThumbnailOrder(
                        keyFile.get_integer("File Browser", "ThumbnailOrder"));
                }
                ...
                if (keyFile.has_key("File Browser", "BrowserShowsExpComp")) {
                    fbShowExpComp = keyFile.get_boolean("File Browser",
                                                        "BrowserShowsExpComp");
                }

#ifndef WIN32
                if (keyFile.has_key("File Browser", "BrowserShowsHidden")) {
                    fbShowHidden = keyFile.get_boolean("File Browser",
                                                       "BrowserShowsHidden");
                }
#endif
                ...
                if (keyFile.has_key("File Browser", "ParseExtensions")) {
                    auto l = keyFile.get_string_list("File Browser",
                                                     "ParseExtensions");
                    if (!l.empty()) {
                        parseExtensions = l;
                    }
                ...
                if (keyFile.has_key("File Browser", "ThumbnailInterpolation")) {
                    thumbInterp = keyFile.get_integer("File Browser",
                ...
                if (keyFile.has_key("File Browser", "ThumbnailZoomRatios")) {
                    thumbnailZoomRatios = keyFile.get_double_list(
                        "File Browser", "ThumbnailZoomRatios");
                }

                if (keyFile.has_key("File Browser", "OverlayedFileNames")) {
                    overlayedFileNames = keyFile.get_boolean(
                        "File Browser", "OverlayedFileNames");
                }

                if (keyFile.has_key("File Browser",
                ...
                if (keyFile.has_key("File Browser", "FilmStripShowFileNames")) {
                    filmStripShowFileNames = keyFile.get_boolean(
                        "File Browser", "FilmStripShowFileNames");
                }

                if (keyFile.has_key("File Browser", "HighlightSelected")) {
                    highlight_selected_thumbnails = keyFile.get_boolean(
                        "File Browser", "HighlightSelected");
                }

                if (keyFile.has_key("File Browser",
                                    "InternalThumbIfUntouched")) {
                    internalThumbIfUntouched = keyFile.get_boolean(
                        "File Browser", "InternalThumbIfUntouched");
                }

                // if (keyFile.has_key("File Browser", "menuGroupRank")) {
                //     menuGroupRank = keyFile.get_boolean("File Browser",
                //     "menuGroupRank");
                // }

                // if (keyFile.has_key("File Browser", "menuGroupLabel")) {
                //     menuGroupLabel = keyFile.get_boolean("File Browser",
                //     "menuGroupLabel");
                // }

                // if (keyFile.has_key("File Browser",
                // "menuGroupFileOperations")) {
                //     menuGroupFileOperations = keyFile.get_boolean("File
                //     Browser", "menuGroupFileOperations");
                // }

                // if (keyFile.has_key("File Browser",
                // "menuGroupProfileOperations")) {
                //     menuGroupProfileOperations = keyFile.get_boolean("File
                //     Browser", "menuGroupProfileOperations");
                // }

                // if (keyFile.has_key("File Browser", "menuGroupExtProg")) {
                //     menuGroupExtProg = keyFile.get_boolean("File Browser",
                //     "menuGroupExtProg");
                // }
                ...
                recentFolders.reserve(
                    maxRecentFolders +
                    10); // reserve some more than maxRecentFolders, because at
                         // runtime it stores more than that

                if (keyFile.has_key("File Browser", "RecentFolders")) {
                    recentFolders = keyFile.get_string_list("File Browser",
                                                            "RecentFolders");
                }

                if (keyFile.has_key("File Browser", "ThumbnailRatingMode")) {
                    auto s = keyFile.get_string("File Browser",
                                                "ThumbnailRatingMode");
                    if (s == "procparams") {
            ...
            if (keyFile.has_group("Clipping Indication")) {
                if (keyFile.has_key("Clipping Indication",
                                    "HighlightThreshold")) {
                ...
                if (keyFile.has_key("Clipping Indication", "ShadowThreshold")) {
                    shadowThreshold = keyFile.get_integer("Clipping Indication",
                                                          "ShadowThreshold");
                }
            }
            ...
            if (keyFile.has_group("Performance")) {
                if (keyFile.has_key("Performance", "RgbDenoiseThreadLimit")) {
                    rgbDenoiseThreadLimit = keyFile.get_integer(
                        "Performance", "RgbDenoiseThreadLimit");
                }

                if (keyFile.has_key("Performance", "ClutCacheSize")) {
                    clutCacheSize =
                        keyFile.get_integer("Performance", "ClutCacheSize");
                }

                if (keyFile.has_key("Performance", "MaxInspectorBuffers")) {
                    maxInspectorBuffers = keyFile.get_integer(
                        "Performance", "MaxInspectorBuffers");
                }
                ...
                if (keyFile.has_key("Performance", "DenoiseZoomedOut")) {
                    denoiseZoomedOut =
                        keyFile.get_boolean("Performance", "DenoiseZoomedOut");
                }
                ...
                if (keyFile.has_key("Performance", "ThumbUpdateThreadLimit")) {
                    rtSettings.thread_pool_size = keyFile.get_integer(
                        "Performance", "ThumbUpdateThreadLimit");
                }

                if (keyFile.has_key("Performance", "ThumbDelayUpdate")) {
                    thumb_delay_update =
                        keyFile.get_boolean("Performance", "ThumbDelayUpdate");
                }

                if (keyFile.has_key("Performance", "ThumbLazyCaching")) {
                    thumb_lazy_caching =
                        keyFile.get_boolean("Performance", "ThumbLazyCaching");
                }

                if (keyFile.has_key("Performance", "ThumbCacheProcessed")) {
                    thumb_cache_processed = keyFile.get_boolean(
                ...
                if (keyFile.has_key("Performance", "RAWImageIOCacheSize")) {
                    rtSettings.imgio_raw_cache_size = keyFile.get_integer(
                        "Performance", "RAWImageIOCacheSize");
                }

                if (keyFile.has_key("Performance",
                                    "PreviewResamplingQuality")) {
                    preview_resampling_quality =
                        PreviewResamplingQuality(keyFile.get_integer(
                            "Performance", "PreviewResamplingQuality"));
                }
            }
            ...
            if (keyFile.has_group("Inspector")) {
                ...
                if (keyFile.has_key("Inspector", "ZoomFit")) {
                    thumbnail_inspector_zoom_fit =
                        keyFile.get_boolean("Inspector", "ZoomFit");
                }

                if (keyFile.has_key("Inspector", "ShowInfo")) {
                    thumbnail_inspector_show_info =
                        keyFile.get_boolean("Inspector", "ShowInfo");
                }

                if (keyFile.has_key("Inspector", "ShowHistogram")) {
                    thumbnail_inspector_show_histogram =
                        keyFile.get_boolean("Inspector", "ShowHistogram");
                }

                if (keyFile.has_key("Inspector", "EnableCMS")) {
                    thumbnail_inspector_enable_cms =
                        keyFile.get_boolean("Inspector", "EnableCMS");
                }

                if (keyFile.has_key("Inspector", "BrowserWidth")) {
                    browser_width_for_inspector =
                        keyFile.get_integer("Inspector", "BrowserWidth");
                }
            ...
            if (keyFile.has_group("GUI")) {
                ...
                if (keyFile.has_key("GUI", "MeowFullScreen")) {
                    meowFullScreen =
                        keyFile.get_boolean("GUI", "MeowFullScreen");
                }
                ...
                if (keyFile.has_key("GUI", "WindowMaximized")) {
                    windowMaximized =
                        keyFile.get_boolean("GUI", "WindowMaximized");
                }

                if (keyFile.has_key("GUI", "DetailWindowWidth")) {
                    detailWindowWidth =
                        keyFile.get_integer("GUI", "DetailWindowWidth");
                }
                ...
                if (keyFile.has_key("GUI", "DirBrowserWidth")) {
                    dirBrowserWidth =
                        keyFile.get_integer("GUI", "DirBrowserWidth");
                }

                if (keyFile.has_key("GUI", "DirBrowserHeight")) {
                    dirBrowserHeight =
                        keyFile.get_integer("GUI", "DirBrowserHeight");
                }
                ...
                if (keyFile.has_key("GUI", "DirBrowserSingleClick")) {
                    dir_browser_single_click =
                        keyFile.get_boolean("GUI", "DirBrowserSingleClick");
                }

                if (keyFile.has_key("GUI", "PreferencesWidth")) {
                    preferencesWidth =
                        keyFile.get_integer("GUI", "PreferencesWidth");
                }

                if (keyFile.has_key("GUI", "PreferencesHeight")) {
                    preferencesHeight =
                        keyFile.get_integer("GUI", "PreferencesHeight");
                }

                if (keyFile.has_key("GUI", "SaveAsDialogWidth")) {
                    saveAsDialogWidth =
                        keyFile.get_integer("GUI", "SaveAsDialogWidth");
                }
                ...
                if (keyFile.has_key("GUI", "ToolPanelWidth")) {
                    toolPanelWidth =
                        keyFile.get_integer("GUI", "ToolPanelWidth");
                }
                ...
                if (keyFile.has_key("GUI", "InspectorDirPanelOpened")) {
                    inspectorDirPanelOpened =
                        keyFile.get_boolean("GUI", "InspectorDirPanelOpened");
                }

                if (keyFile.has_key("GUI", "HistoryPanelWidth")) {
                    historyPanelWidth =
                        keyFile.get_integer("GUI", "HistoryPanelWidth");
                }
                ...
                if (keyFile.has_key("GUI", "PanAccelFactor")) {
                    panAccelFactor =
                        keyFile.get_integer("GUI", "PanAccelFactor");
                }
                ...
                if (keyFile.has_key("GUI", "FilmStripBottom")) {
                    filmstripBottom =
                        keyFile.get_boolean("GUI", "FilmStripBottom");
                }
                ...
                if (keyFile.has_key("GUI", "ToolPanelsExpanded")) {
                    tpOpen =
                        keyFile.get_integer_list("GUI", "ToolPanelsExpanded");
                }

                if (keyFile.has_key("GUI", "ToolPanelsExpandedAutoSave")) {
                    autoSaveTpOpen = keyFile.get_boolean(
                        "GUI", "ToolPanelsExpandedAutoSave");
                }

                if (keyFile.has_key("GUI", "MultiDisplayMode")) {
                    multiDisplayMode =
                        keyFile.get_integer("GUI", "MultiDisplayMode");
                }
                ...
                //      cutOverlayBrush = keyFile.get_double_list("GUI",
                //      "CutOverlayBrush");
                //  }

                // if (keyFile.has_key("GUI", "NavGuideBrush")) {
                //     navGuideBrush = keyFile.get_double_list("GUI",
                //     "NavGuideBrush");
                // }

                if (keyFile.has_key("GUI", "HistogramPosition")) {
                    histogramPosition =
                        keyFile.get_integer("GUI", "HistogramPosition");
                }
                ...
                if (keyFile.has_key("GUI", "HistogramGreen")) {
                    histogramGreen =
                        keyFile.get_boolean("GUI", "HistogramGreen");
                }
                ...
                if (keyFile.has_key("GUI", "HistogramChroma")) {
                    histogramChroma =
                        keyFile.get_boolean("GUI", "HistogramChroma");
                }

                if (keyFile.has_key("GUI", "HistogramRAW")) {
                    // histogramRAW = keyFile.get_boolean("GUI",
                    // "HistogramRAW");
                ...
                if (keyFile.has_key("GUI", "HistogramHeight")) {
                    histogramHeight =
                        keyFile.get_integer("GUI", "HistogramHeight");
                }

                if (keyFile.has_key("GUI", "HistogramDrawMode")) {
                    histogramDrawMode =
                        keyFile.get_integer("GUI", "HistogramDrawMode");
                }

                if (keyFile.has_key("GUI", "HistogramScalingFactor")) {
                    histogram_scaling_factor =
                        keyFile.get_double("GUI", "HistogramScalingFactor");
                }

                if (keyFile.has_key("GUI", "NavigatorRGBUnit")) {
                    navRGBUnit = (NavigatorUnit)keyFile.get_integer(
                        "GUI", "NavigatorRGBUnit");
                }

                if (keyFile.has_key("GUI", "NavigatorLCHUnit")) {
                    navLCHUnit = (NavigatorUnit)keyFile.get_integer(
                        "GUI", "NavigatorLCHUnit");
                }
                ...
                if (keyFile.has_key("GUI", "HistogramShowOptionButtons")) {
                    histogramShowOptionButtons = keyFile.get_boolean(
                        "GUI", "HistogramShowOptionButtons");
                }
                ...
                if (keyFile.has_key("GUI", "FileBrowserToolbarSingleRow")) {
                    FileBrowserToolbarSingleRow = keyFile.get_boolean(
                        "GUI", "FileBrowserToolbarSingleRow");
                }

                if (keyFile.has_key("GUI", "HideTPVScrollbar")) {
                    hideTPVScrollbar =
                        keyFile.get_boolean("GUI", "HideTPVScrollbar");
                }
                ...
                if (keyFile.has_key("GUI", "CurveBBoxPosition")) {
                    curvebboxpos =
                        keyFile.get_integer("GUI", "CurveBBoxPosition");
                }

                if (keyFile.has_key("GUI", "ToolPanelsDisable")) {
                    toolpanels_disable =
                        keyFile.get_boolean("GUI", "ToolPanelsDisable");
                }
            ...
            if (keyFile.has_group("Color Management")) {
                ...
                if (keyFile.has_key("Color Management",
                                    "MonitorICCDirectory")) {
                    rtSettings.monitorIccDirectory = keyFile.get_string(
                        "Color Management", "MonitorICCDirectory");
                } else {
                    rtSettings.monitorIccDirectory = rtSettings.iccDirectory;
                }
                ...
                if (keyFile.has_key("Color Management", "PrinterProfile")) {
                    rtSettings.printerProfile = keyFile.get_string(
                        "Color Management", "PrinterProfile");
                }

                if (keyFile.has_key("Color Management", "MonitorProfile")) {
                    rtSettings.monitorProfile = keyFile.get_string(
                        "Color Management", "MonitorProfile");
                }

                if (keyFile.has_key("Color Management", "AutoMonitorProfile")) {
                    rtSettings.autoMonitorProfile = keyFile.get_boolean(
                        "Color Management", "AutoMonitorProfile");
                }
                ...
                if (keyFile.has_key("Color Management",
                    ...
                    whiteBalanceSpotSize = keyFile.get_integer(
                        "Color Management", "WhiteBalanceSpotSize");
                }

                if (keyFile.has_key("Color Management", "ClutsDirectory")) {
                    clutsDir = keyFile.get_string("Color Management",
                                                  "ClutsDirectory");
                }
                ...
                if (keyFile.has_key("Color Management", "OSMonitorProfile")) {
                    auto s =
                        keyFile
                            .get_string("Color Management", "OSMonitorProfile")
                            .lowercase();
                    if (s == "displayp3") {
            ...
            if (keyFile.has_group("Sounds")) {
                ...
                if (keyFile.has_key("Sounds", "BatchQueueDone")) {
                    sndBatchQueueDone =
                        keyFile.get_string("Sounds", "BatchQueueDone");
                }
            ...
            if (keyFile.has_group("Fast Export")) {
                if (keyFile.has_key("Fast Export", "fastexport_resize_width")) {
                    fastexport_resize_width = keyFile.get_integer(
                        "Fast Export", "fastexport_resize_width");
                } else if (keyFile.has_key("Fast Export", "MaxWidth")) {
                    fastexport_resize_width =
                        keyFile.get_integer("Fast Export", "MaxWidth");
                }

                if (keyFile.has_key("Fast Export",
                                    "fastexport_resize_height")) {
                    fastexport_resize_height = keyFile.get_integer(
                        "Fast Export", "fastexport_resize_height");
                } else if (keyFile.has_key("Fast Export", "MaxHeight")) {
                    fastexport_resize_height =
                        keyFile.get_integer("Fast Export", "MaxHeight");
                }
            }

            if (keyFile.has_group("Dialogs")) {
                safeDirGet(keyFile, "Dialogs", "LastIccDir", lastIccDir);
                safeDirGet(keyFile, "Dialogs", "LastDarkframeDir",
                           lastDarkframeDir);
                safeDirGet(keyFile, "Dialogs", "LastFlatfieldDir",
                           lastFlatfieldDir);
                safeDirGet(keyFile, "Dialogs", "LastRgbCurvesDir",
                           lastRgbCurvesDir);
                safeDirGet(keyFile, "Dialogs", "LastLabCurvesDir",
                           lastLabCurvesDir);
                safeDirGet(keyFile, "Dialogs", "LastPFCurvesDir",
                           lastPFCurvesDir);
                safeDirGet(keyFile, "Dialogs", "LastHsvCurvesDir",
                           lastHsvCurvesDir);

                safeDirGet(keyFile, "Dialogs", "LastToneCurvesDir",
                           lastToneCurvesDir);
                safeDirGet(keyFile, "Dialogs", "LastProfilingReferenceDir",
                           lastProfilingReferenceDir);
                safeDirGet(keyFile, "Dialogs", "LastLensProfileDir",
                           lastLensProfileDir);
                safeDirGet(keyFile, "Dialogs", "LastICCProfCreatorDir",
                           lastICCProfCreatorDir);
                safeDirGet(keyFile, "Dialogs", "LastSessionAddDir",
                           last_session_add_dir);
                safeDirGet(keyFile, "Dialogs", "LastSessionLoadSaveDir",
                           last_session_loadsave_dir);
                safeDirGet(keyFile, "Dialogs", "LastCopyMovePath",
                           renaming.basedir);
                safeDirGet(keyFile, "Dialogs", "LastRenamingLoadSaveDir",
                           last_renaming_loadsave_dir);

                if (keyFile.has_key("Dialogs", "GimpPluginShowInfoDialog")) {
                    gimpPluginShowInfoDialog = keyFile.get_boolean(
                        "Dialogs", "GimpPluginShowInfoDialog");
                }
            }
            ...
            if (keyFile.has_group("Metadata")) {
                ...
                if (keyFile.has_key("Metadata", "ExiftoolPath")) {
                    rtSettings.exiftool_path =
                        keyFile.get_string("Metadata", "ExiftoolPath");
                }
                if (keyFile.has_key("Metadata", "ShowExiftoolMakernotes")) {
                    show_exiftool_makernotes = keyFile.get_boolean(
                        "Metadata", "ShowExiftoolMakernotes");
                }
            }
            ...
            if (keyFile.has_group("False Colors Map")) {
                ...
                if (keyFile.has_key(g, "ClippedHighlights")) {
                    clipped_highlights_color =
                        keyFile.get_string(g, "ClippedHighlights");
                }
                if (keyFile.has_key(g, "ClippedShadows")) {
                    clipped_shadows_color =
                        keyFile.get_string(g, "ClippedShadows");
                }
            }

            renaming.load(keyFile);
            if (keyFile.has_group("Renaming") &&
                keyFile.has_key("Renaming", "Remember")) {
            ...
            if (keyFile.has_group("ExifFilterSettings")) {
                ...
                if (keyFile.has_key(g, "Remember")) {
                    remember_exif_filter_settings =
                        keyFile.get_boolean(g, "Remember");
                }
            ...
            if (keyFile.has_key("Output", "BatchQueueUseProfile")) {
                default_export_profile_info.enabled =
                    keyFile.get_boolean("Output", "BatchQueueUseProfile");
            }

            if (keyFile.has_key("Output", "BatchQueueProfile")) {
                default_export_profile_info.profile =
                    keyFile.get_string("Output", "BatchQueueProfile");
            }
        ...
        Glib::ustring msg =
            Glib::ustring::compose("Options::readFromFile / Error code %1 "
                                   "while reading values from \"%2\":\n%3",
        ...
        Glib::ustring msg =
            Glib::ustring::compose("Options::readFromFile / Unknown exception "
                                   "while trying to load \"%1\"!",
                                   fname);
...
void Options::saveToFile(Glib::ustring fname)
{
    ...
    try {
        ...
        keyFile.set_string("General", "DarkFramesPath",
                           rtSettings.darkFramesPath);
        keyFile.set_string("General", "FlatFieldsPath",
                           rtSettings.flatFieldsPath);
        keyFile.set_integer("General", "Verbose", rtSettings.verbose);
        keyFile.set_integer("General", "ErrorMessageDuration",
                            error_message_duration);
        keyFile.set_integer("General", "MaxErrorMessages", max_error_messages);
        keyFile.set_integer("General", "EditorKeyboardScrollStep",
                            editor_keyboard_scroll_step);
        keyFile.set_integer("General", "AdjusterShortcutScrollWheelFactor",
                            adjuster_shortcut_scrollwheel_factor);
        ...
        keyFile.set_integer("External Editor", "OutputDir",
                            int(editor_out_dir));
        keyFile.set_string("External Editor", "CustomOutputDir",
                           editor_custom_out_dir);
        ...
        keyFile.set_boolean("File Browser", "BrowserShowsExif",
                            fbShowBasicExif);
        keyFile.set_boolean("File Browser", "BrowserShowsExpComp",
                            fbShowExpComp);
#ifndef WIN32
        ...
        keyFile.set_integer("File Browser", "ThumbnailSizeQueue",
                            thumbSizeQueue);
        keyFile.set_integer("File Browser", "SameThumbSize", sameThumbSize);
        keyFile.set_integer("File Browser", "ThumbnailOrder",
                            int(thumbnailOrder));
        keyFile.set_integer("File Browser", "MaxPreviewHeight",
                            maxThumbnailHeight);
        keyFile.set_integer("File Browser", "MaxPreviewWidth",
                            maxThumbnailWidth);
        ...
        keyFile.set_integer_list("File Browser", "ParseExtensionsEnabled",
                                 pextena);
        ...
        keyFile.set_boolean("File Browser", "OverlayedFileNames",
                            overlayedFileNames);
        ...
        keyFile.set_boolean("File Browser", "HighlightSelected",
                            highlight_selected_thumbnails);
        keyFile.set_boolean("File Browser", "InternalThumbIfUntouched",
                            internalThumbIfUntouched);
        // keyFile.set_boolean("File Browser", "menuGroupRank", menuGroupRank);
        // keyFile.set_boolean("File Browser", "menuGroupLabel",
        // menuGroupLabel); keyFile.set_boolean("File Browser",
        // "menuGroupFileOperations", menuGroupFileOperations);
        // keyFile.set_boolean("File Browser", "menuGroupProfileOperations",
        // menuGroupProfileOperations); keyFile.set_boolean("File Browser",
        // "menuGroupExtProg", menuGroupExtProg);
        keyFile.set_integer("File Browser", "MaxRecentFolders",
                            maxRecentFolders);
        {
        ...
        default: // ThumbnailRatingMode::PROCPARAMS
            keyFile.set_string("File Browser", "ThumbnailRatingMode",
                               "procparams");
            break;
        }
        keyFile.set_integer("Clipping Indication", "HighlightThreshold",
                            highlightThreshold);
        keyFile.set_integer("Clipping Indication", "ShadowThreshold",
                            shadowThreshold);
        ...
        keyFile.set_boolean("Performance", "SerializeTiffRead",
                            serializeTiffRead);
        keyFile.set_boolean("Performance", "DenoiseZoomedOut",
                            denoiseZoomedOut);
        keyFile.set_integer("Performance", "ThumbUpdateThreadLimit",
                            rtSettings.thread_pool_size);
        keyFile.set_boolean("Performance", "ThumbDelayUpdate",
                            thumb_delay_update);
        keyFile.set_boolean("Performance", "ThumbLazyCaching",
                            thumb_lazy_caching);
        keyFile.set_boolean("Performance", "ThumbCacheProcessed",
        ...
        keyFile.set_integer("Performance", "PreviewResamplingQuality",
                            int(preview_resampling_quality));

        keyFile.set_integer("Inspector", "Mode",
                            int(rtSettings.thumbnail_inspector_mode));
        keyFile.set_integer("Inspector", "RawCurve",
                            int(rtSettings.thumbnail_inspector_raw_curve));
        keyFile.set_boolean("Inspector", "ZoomFit",
                            thumbnail_inspector_zoom_fit);
        keyFile.set_boolean("Inspector", "ShowInfo",
                            thumbnail_inspector_show_info);
        keyFile.set_boolean("Inspector", "ShowHistogram",
                            thumbnail_inspector_show_histogram);
        keyFile.set_boolean("Inspector", "EnableCMS",
                            thumbnail_inspector_enable_cms);
        keyFile.set_integer("Inspector", "BrowserWidth",
                            browser_width_for_inspector);
        ...
        keyFile.set_boolean("Output", "TiffUncompressed",
                            saveFormat.tiffUncompressed);
        ...
        keyFile.set_integer("Output", "JpegQualityBatch",
                            saveFormatBatch.jpegQuality);
        keyFile.set_integer("Output", "JpegSubSampBatch",
                            saveFormatBatch.jpegSubSamp);
        ...
        keyFile.set_boolean("Output", "TiffFloatBatch",
                            saveFormatBatch.tiffFloat);
        keyFile.set_boolean("Output", "TiffUncompressedBatch",
                            saveFormatBatch.tiffUncompressed);
        keyFile.set_boolean("Output", "SaveProcParamsBatch",
                            saveFormatBatch.saveParams);
        ...
        keyFile.set_boolean("Output", "OverwriteOutputFile",
                            overwriteOutputFile);
        // keyFile.set_boolean("Output", "BatchQueueUseProfile",
        // batch_queue_use_profile); keyFile.set_string("Output",
        // "BatchQueueProfile", batch_queue_profile_path);
        keyFile.set_integer("Output", "ProcParamsAutosaveInterval",
                            sidecar_autosave_interval);

        keyFile.set_string("Profiles", "Directory", profilePath);
        keyFile.set_boolean("Profiles", "UseBundledProfiles",
                            useBundledProfiles);
        keyFile.set_string("Profiles", "LoadSaveProfilePath",
                           loadSaveProfilePath);
        ...
        keyFile.set_integer("Profiles", "LoadParamsFromLocation",
                            paramsLoadLocation);
        keyFile.set_boolean("Profiles", "EmbedParamsInMetadata",
                            params_out_embed);
        keyFile.set_boolean("Profiles", "ParamsSidecarStripExtension",
                            params_sidecar_strip_extension);
        ...
        keyFile.set_integer("GUI", "DirBrowserSingleClick",
                            dir_browser_single_click);
        ...
        keyFile.set_integer("GUI", "BrowserToolPanelWidth",
                            browserToolPanelWidth);
        keyFile.set_integer("GUI", "BrowserToolPanelHeight",
                            browserToolPanelHeight);
        keyFile.set_boolean("GUI", "BrowserToolPanelOpened",
                            browserToolPanelOpened);
        keyFile.set_boolean("GUI", "EditorFilmStripOpened",
                            editorFilmStripOpened);
        keyFile.set_boolean("GUI", "BrowserDirPanelOpened",
                            browserDirPanelOpened);
        keyFile.set_boolean("GUI", "InspectorDirPanelOpened",
                            inspectorDirPanelOpened);
        ...
        keyFile.set_boolean("GUI", "ShowClippedHighlights",
                            showClippedHighlights);
        ...
        keyFile.set_boolean("GUI", "ShowFilmStripToolBar",
                            showFilmStripToolBar);
        ...
        keyFile.set_integer("Color Management", "PrinterIntent",
                            rtSettings.printerIntent);
        keyFile.set_boolean("Color Management", "PrinterBPC",
                            rtSettings.printerBPC);

        keyFile.set_string("Color Management", "ICCDirectory",
                           rtSettings.iccDirectory);
        keyFile.set_string("Color Management", "MonitorICCDirectory",
                           rtSettings.monitorIccDirectory);
        ...
        keyFile.set_integer("Color Management", "Intent",
                            rtSettings.monitorIntent);
        keyFile.set_boolean("Color Management", "MonitorBPC",
                            rtSettings.monitorBPC);
        ...
        keyFile.set_integer("Fast Export", "MaxHeight",
                            fastexport_resize_height);
        ...
        keyFile.set_string("Dialogs", "LastICCProfCreatorDir",
                           lastICCProfCreatorDir);
        keyFile.set_string("Dialogs", "LastSessionAddDir",
                           last_session_add_dir);
        keyFile.set_string("Dialogs", "LastSessionLoadSaveDir",
                           last_session_loadsave_dir);
        keyFile.set_string("Dialogs", "LastRenamingLoadSaveDir",
                           last_renaming_loadsave_dir);
        keyFile.set_boolean("Dialogs", "GimpPluginShowInfoDialog",
                            gimpPluginShowInfoDialog);

        keyFile.set_string("Lensfun", "DBDirectory",
                           rtSettings.lensfunDbDirectory);
        ...
        keyFile.set_string("Metadata", "ExiftoolPath",
                           rtSettings.exiftool_path);
        keyFile.set_boolean("Metadata", "ShowExiftoolMakernotes",
                            show_exiftool_makernotes);

        for (auto &p : falseColorsMap) {
            keyFile.set_string("False Colors Map",
                               "IRE_" + std::to_string(p.first), p.second);
        }
        keyFile.set_string("False Colors Map", "ClippedHighlights",
                           clipped_highlights_color);
        keyFile.set_string("False Colors Map", "ClippedShadows",
                           clipped_shadows_color);
        renaming.save(keyFile);
        keyFile.set_integer("Renaming", "Remember", renaming_remember);

        keyFile.set_boolean("ExifFilterSettings", "Remember",
                            remember_exif_filter_settings);
        ...
        for (auto &p : export_profile_map) {
            keyFile.set_string("Export Profile " + p.first, "Profile",
                               p.second.profile);
            keyFile.set_boolean("Export Profile " + p.first, "Enabled",
                                p.second.enabled);
        }
...
//     }
    ...
    if (path != nullptr) {
        ...
        if (!Glib::path_is_absolute(user_config_dir)) {
            Glib::ustring msg = Glib::ustring::compose(
                "Settings path %1 is not absolute", user_config_dir);
            throw Error(msg);
        }
    } else {
#ifdef WIN32
        ...
        if (SHGetSpecialFolderPathW(NULL, pathW, CSIDL_LOCAL_APPDATA, false)) {
            ...
            user_config_dir = Glib::build_filename(
                Glib::ustring(pathA), Glib::ustring(CACHEFOLDERNAME));
        }

#else
        user_config_dir =
            Glib::build_filename(Glib::ustring(g_get_user_config_dir()),
    ...
    // Read the global option file (the one located in the application's base
    // folder)
    ...
    // Modify the path of the cache folder to the one provided in ART_CACHE
    ...
    // No environment variable provided, so falling back to the multi user mode,
    // if enabled
...
#else
        cacheBaseDir =
            Glib::build_filename(Glib::ustring(g_get_user_cache_dir()),
                                 Glib::ustring(CACHEFOLDERNAME));
#endif
    }

    // Read the user option file (the one located somewhere in the user's home
    // folder) Those values supersets those of the global option file
    try {
        ...
        // If the local option file does not exist or is broken, and the local
        // cache folder does not exist, recreate it
        if (!g_mkdir_with_parents(user_config_dir.c_str(), 511)) {
            // Save the option file
            options.saveToFile(
                Glib::build_filename(user_config_dir, "options"));
        }
    }
    ...
    if (options.defProfRaw.empty()) {
        ...
        if (!options.findProfilePath(options.defProfRaw).empty()) {
            if (options.rtSettings.verbose) {
                std::cout << "Default profile for raw images \""
                          << options.defProfRaw << "\" found" << std::endl;
            }
        } else {
    ...
    // We handle languages using a hierarchy of translations.  The top of the
    // hierarchy is default.  This includes a default translation for all items
    // (most likely using simple English).  The next level is the language: for
    // instance, English, French, Chinese, etc.  This file should contain a
    // generic translation for all items which differ from default.  Finally
    // there is the locale.  This is region-specific items which differ from the
    // language file.  These files must be name in the format <Language> (<LC>),
    // where Language is the name of the language which it inherits from, and LC
    // is the local code.  Some examples of this would be English (US) (American
    // English), French (FR) (France French), French (CA) (Canadian French),
    // etc.
    //
    // Each level will only contain the differences between itself and its
    // parent translation.  For instance, English (UK) or English (CA) may
    // include the translation "HISTORY_MSG_34;Avoid Colour Clipping" where
    // English would translate it as "HISTORY_MSG_34;Avoid Color Clipping" (note
    // the difference in the spelling of 'colour').
    //
    // It is important that when naming the translation files, that you stick to
    // the format <Language> or <Language> (<LC>).  We depend on that to figure
    // out which are the parent translations.  Furthermore, there must be a file
    // <Language> for each locale <Language> (<LC>) -- you cannot have 'French
    // (CA)' unless there is a file 'French'.

    Glib::ustring defaultTranslation =
        Glib::build_filename(ART_base_dir, "languages", "default");
    Glib::ustring languageTranslation = "";
    Glib::ustring localeTranslation = "";

    Glib::ustring user_default_translation =
        Glib::build_filename(user_config_dir, "languages", "default");
...
#ifdef ART_USE_CTL
    ctl_default_translation = Glib::build_filename(ART_base_dir, "ctlscripts",
                                                   "languages", "default");
    user_ctl_default_translation = Glib::build_filename(
        user_config_dir, "ctlscripts", "languages", "default");
#endif // ART_USE_CTL
    ...
    if (!options.language.empty()) {
        ...
        if (langPortions.size() >= 1) {
            languageTranslation = Glib::build_filename(
                ART_base_dir, "languages", langPortions.at(0));
            user_language_translation = Glib::build_filename(
                user_config_dir, "languages", langPortions.at(0));

#ifdef ART_USE_CTL
            ctl_language_translation = Glib::build_filename(
                ART_base_dir, "ctlscripts", "languages", langPortions.at(0));
            user_ctl_language_translation = Glib::build_filename(
                user_config_dir, "ctlscripts", "languages", langPortions.at(0));
#endif // ART_USE_CTL
        }

        if (langPortions.size() >= 2) {
            localeTranslation = Glib::build_filename(ART_base_dir, "languages",
                                                     options.language);
            user_locale_translation = Glib::build_filename(
                user_config_dir, "languages", options.language);

#ifdef ART_USE_CTL
            ctl_locale_translation = Glib::build_filename(
                ART_base_dir, "ctlscripts", "languages", options.language);
            user_ctl_locale_translation = Glib::build_filename(
                user_config_dir, "ctlscripts", "languages", options.language);
#endif // ART_USE_CTL
        }
    }

    std::vector<Glib::ustring> fnames = {
        user_locale_translation,       localeTranslation,
        user_ctl_locale_translation,   ctl_locale_translation,
        user_language_translation,     languageTranslation,
        user_ctl_language_translation, ctl_language_translation,
        user_default_translation,      defaultTranslation,
        user_ctl_default_translation,  ctl_default_translation};
...
// Pattern matches "5.1" from "5.1-23-g12345678", when comparing option.version
// to RTVERSION
...
{
    return parsedExtensionsSet.find(ext.lowercase()) !=
           parsedExtensionsSet.end();
}
...
Glib::ustring Options::getICCProfileCopyright()
{
    ...
    return Glib::ustring::compose("Copyright RawTherapee %1, CC0",
                                  now.get_year());
}
