                        ...
                        false, true, true)
{
    ...
    hb->pack_start(*Gtk::manage(new Gtk::Label(
                       M("TP_PREPROCESS_LINEDENOISE_DIRECTION") + ": ")),
                   Gtk::PACK_SHRINK, 0);
    lineDenoiseDirection = Gtk::manage(new MyComboBoxText());
    lineDenoiseDirection->append(
        M("TP_PREPROCESS_LINEDENOISE_DIRECTION_HORIZONTAL"));
    lineDenoiseDirection->append(
        M("TP_PREPROCESS_LINEDENOISE_DIRECTION_VERTICAL"));
    lineDenoiseDirection->append(M("TP_PREPROCESS_LINEDENOISE_DIRECTION_BOTH"));
    lineDenoiseDirection->append(
        M("TP_PREPROCESS_LINEDENOISE_DIRECTION_PDAF_LINES"));
    ...
    dynamicRowNoiseFilter = Gtk::manage(
        new Gtk::CheckButton((M("TP_PREPROCESS_DYNROWNOISEFILTER"))));
    dynamicRowNoiseFilter->show();
    dynamicRowNoiseFilter->signal_toggled().connect(
        sigc::mem_fun(*this, &BayerPreProcess::dynamicRowNoiseFilterChanged),
        true);
...
void BayerPreProcess::read(const rtengine::procparams::ProcParams *pp)
{
    ...
    dynamicRowNoiseFilter->set_active(
        pp->raw.bayersensor.dynamicRowNoiseFilter);
...
void BayerPreProcess::write(rtengine::procparams::ProcParams *pp)
{
    ...
    pp->raw.bayersensor.dynamicRowNoiseFilter =
        dynamicRowNoiseFilter->get_active();
}
...
{
    if (listener && getEnabled()) {
        listener->panelChanged(EvDynamicRowNoiseFilter,
                               dynamicRowNoiseFilter->get_active()
                                   ? M("GENERAL_ENABLED")
                                   : M("GENERAL_DISABLED"));
    }
