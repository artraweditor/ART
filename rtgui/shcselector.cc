      ...
      coloredBar(RTO_Left2Right)
{
    ...
    // TODO: This is a hack :) ; change this name to a specific one and create a
    // new entry in all CSS theme files
...
void SHCSelector::updateBackBuffer()
{
    ...
    setDrawRectangle(Cairo::FORMAT_ARGB32, 0, 0, get_width(), get_height(),
                     true);
    ...
    if (is_sensitive() && coloredBar.canGetColors()) {
        ...
        coloredBar.setDrawRectangle(leftMargin + 1 * (int)s, 1 * (int)s,
                                    w - 2 * (int)s,
                                    coloredBarHeight - 2 * (int)s);
        // that we're displaying here
        coloredBar.expose(*this, cr);
    } else {
        style->render_background(cr, leftMargin + 1 * (int)s, 1 * (int)s,
                                 w - 2 * (int)s, coloredBarHeight - 2 * (int)s);
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        style->render_slider(
            cr,
            (double)leftMargin + 1. * s + ((double)w - 2. * s) * positions[i] -
                (double)hwslider,
            (double)vb * s, wslider, (double)h - (double)vb * s,
    ...
    if (movingPosition >= 0) {
        ...
        offset = positions[i] > 0.5 ? -layout_width - 1 * (int)s - hwslider
                                    : 1 * (int)s + hwslider;
        ...
        cr->move_to(
            (double)leftMargin + (double)w * positions[i] + (double)offset, 0.);
...
bool SHCSelector::on_button_press_event(GdkEventButton *event)
{
    ...
    for (int i = 0; i < 3; i++) {
        double currPos =
            double(leftMargin) + 1. * s + (w - 2. * s) * positions[i];
        double hwslider = wslider / 2.;
