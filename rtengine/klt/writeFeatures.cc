...
void KLTWriteFeatureListToPPM(KLT_FeatureList featurelist,
                              KLT_PixelType *greyimg, int ncols, int nrows,
                              const char *filename)
{
...
static FILE *_printSetupBin(const char *fname) /* Input: filename */
{
    FILE *fp;
    if (fname == nullptr) {
...
static void _printNhyphens(FILE *fp, int n)
{
    int i;
    for (i = 0; i < n; i++)
        fprintf(fp, "-");
...
static void _printInteger(FILE *fp, int integer, int width)
{
    char fmt[80];
...
static KLT_BOOL _isCharInString(const char c, const char *str)
{
...
static int _findStringWidth(const char *str)
{
    int width = 0;
    int add;
...
static void _printHeader(FILE *fp, const char *format, structureType id,
                         int nFrames, int nFeatures)
{
    ...
    switch (id) {
    case FEATURE_LIST:
        fprintf(fp, "KLT Feature List\n");
        break;
    case FEATURE_HISTORY:
        fprintf(fp, "KLT Feature History\n");
        break;
    case FEATURE_TABLE:
        fprintf(fp, "KLT Feature Table\n");
        break;
    }
    ...
    case FEATURE_HISTORY:
        ...
        break;
    case FEATURE_TABLE:
        fprintf(fp, "nFrames = %d, nFeatures = %d\n\n", nFrames, nFeatures);
        break;
    }

    switch (id) {
    case FEATURE_LIST:
        fprintf(fp, "feature | (x,y)=val\n");
        fprintf(fp, "--------+-");
        _printNhyphens(fp, width);
        fprintf(fp, "\n");
        break;
    case FEATURE_HISTORY:
        fprintf(fp, "frame | (x,y)=val\n");
        fprintf(fp, "------+-");
        _printNhyphens(fp, width);
        fprintf(fp, "\n");
        break;
    case FEATURE_TABLE:
        fprintf(fp, "feature |          frame\n");
        fprintf(fp, "        |");
        for (i = 0; i < nFrames; i++)
            _printInteger(fp, i, width);
        fprintf(fp, "\n--------+-");
        for (i = 0; i < nFrames; i++)
            _printNhyphens(fp, width);
        fprintf(fp, "\n");
...
static void _printFeatureTxt(FILE *fp, KLT_Feature feat, const char *format,
                             const char type)
{
...
static void _printFeatureBin(FILE *fp, KLT_Feature feat)
{
...
static void _printShutdown(FILE *fp)
{
...
void KLTWriteFeatureList(KLT_FeatureList fl, const char *fname, const char *fmt)
{
    FILE *fp;
    char format[100];
    char type;
    ...
    if (KLT_verbose >= 1 && fname != nullptr) {
        fprintf(stderr, "(KLT) Writing feature list to %s file: '%s'\n",
...
void KLTWriteFeatureHistory(KLT_FeatureHistory fh, const char *fname,
                            const char *fmt)
{
    FILE *fp;
    char format[100];
    ...
    if (KLT_verbose >= 1 && fname != nullptr) {
        fprintf(stderr, "(KLT) Writing feature history to %s file: '%s'\n",
                (fmt == nullptr ? "binary" : "text"), fname);
    }
...
static structureType _readHeader(FILE *fp, int *nFrames, int *nFeatures,
                                 KLT_BOOL *binary)
{
#define LINELENGTH 100
    ...
    while (strcmp(line, warning_line) != 0) {
        ...
        if (feof(fp)) {
            KLTError(
                "(_readFeatures) File is corrupted -- Couldn't find line:\n"
                "\t%s\n",
                warning_line);
            exit(1);
        }
    ...
    if (strcmp(line, "KLT Feature List\n") == 0)
        ...
        KLTError(
            "(_readFeatures) File is corrupted -- (Not 'KLT Feature List', "
    ...
    if (id == FEATURE_LIST) {
        if (strcmp(line, "nFeatures") != 0) {
            KLTError("(_readFeatures) File is corrupted -- "
                     "(Expected 'nFeatures', found '%s' instead)",
                     line);
            exit(1);
        }
    } else if (strcmp(line, "nFrames") != 0) {
        KLTError("(_readFeatures) File is corrupted -- "
                 "(Expected 'nFrames', found '%s' instead)",
                 line);
        exit(1);
    }
    fscanf(fp, "%s", line);
    ...
    if (id == FEATURE_TABLE) {
        ...
        if (strcmp(line, ",") != 0) {
            KLTError("(_readFeatures) File '%s' is corrupted -- "
                     "(Expected 'comma', found '%s' instead)",
                     line);
            exit(1);
        }
        fscanf(fp, "%s", line);
        if (strcmp(line, "nFeatures") != 0) {
            KLTError("(_readFeatures) File '%s' is corrupted -- "
                     "(2 Expected 'nFeatures ', found '%s' instead)",
                     line);
            exit(1);
        }
        fscanf(fp, "%s", line);
...
static void _readFeatureTxt(FILE *fp, KLT_Feature feat)
{
    while (fgetc(fp) != '(')
        ;
...
static void _readFeatureBin(FILE *fp, KLT_Feature feat)
{
...
KLT_FeatureList KLTReadFeatureList(KLT_FeatureList fl_in, const char *fname)
{
    FILE *fp;
    KLT_FeatureList fl;
    int nFeatures;
...
KLT_FeatureHistory KLTReadFeatureHistory(KLT_FeatureHistory fh_in,
                                         const char *fname)
{
    FILE *fp;
    ...
    if (KLT_verbose >= 1)
        fprintf(stderr, "(KLT) Reading feature history from '%s'\n", fname);
...
KLT_FeatureTable KLTReadFeatureTable(KLT_FeatureTable ft_in, const char *fname)
{
    FILE *fp;
    KLT_FeatureTable ft;
    int nFrames;
    ...
    if (KLT_verbose >= 1)
        fprintf(stderr, "(KLT) Reading feature table from '%s'\n", fname);
    ...
    if (ft_in == nullptr) {
        ...
        if (ft->nFrames != nFrames || ft->nFeatures != nFeatures) {
            KLTError("(KLTReadFeatureTable) The feature table passed "
                     ...
                     "features as the feature table in file '%s' ",
                     fname);
            exit(1);
        }
    }
