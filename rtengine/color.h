...
public:
    ...
    typedef enum InterpolationPath {
        IP_SHORTEST, /// Interpolate color using the shortest path between 2
                     /// hues
        IP_LONGEST,  /// Interpolate color using the longest path between 2 hues
    } eInterpolationPath;

    typedef enum InterpolationDirection {
        ID_UP,  /// Interpolate color by increasing the hue value, crossing the
                /// upper limit
        ID_DOWN /// Interpolate color by decreasing the hue value, crossing the
                /// lower limit
    ...
    static inline void rgb2slfloat(float r, float g, float b, float &s,
                                   float &l)
    {
    ...
    static inline void hsl2rgbfloat(float h, float s, float l, float &r,
                                    float &g, float &b)
    {
     ...
     */
    static void hsl2rgb01(float h, float s, float l, float &r, float &g,
                          float &b);
     ...
     * @param v value channel [0 ; 1] (return value)
     */
    static void rgb2hsv01(float r, float g, float b, float &h, float &s,
                          float &v);
    ...
    static void xyz2r(float x, float y, float z, float &r,
                      const double rgb_xyz[3][3]);
    static void xyz2rgb(float x, float y, float z, float &r, float &g, float &b,
     ...
     */
    static void Lab2Yuv(float L, float a, float b, float &Y, float &u,
                        float &v);
     ...
     * value)
     */
    static void Yuv2Lab(float Y, float u, float v, float &L, float &a, float &b,
                        const double wp[3][3]);
    ...
    /**
     * @brief Convert 'c' and 'h' channels of the Lch color space to the 'a' and
     * 'b' channels of the L*a*b color space (channel 'L' is identical [0 ;
     * 32768])
     * @param c 'c' channel value, in [0 ; 42000]
     * @param h 'h' channel value, in [-PI ; +PI]
     * @param a 'a' channel [-42000 ; +42000] ; can be more than 42000 (return
     * value)
     * @param b 'b' channel [-42000 ; +42000] ; can be more than 42000 (return
     * value)
     */
    static void Lch2Lab(float c, float h, float &a, float &b);

    /**
     * @brief Convert the 'u' and 'v' channels of the Luv color space to 'c' and
     * 'h' channels of the Lch color space ('L' channel is identical)
    ...
    /**
     * @brief Convert 'c' and 'h' channels of the Lch color space to the 'u' and
     * 'v' channels of the Luv color space ('L' channel is identical)
     ...
     * @param u 'u' channel [-42000 ; 42000] ; can be more than 42000 (return
     * value)
     * @param v 'v' channel [-42000 ; 42000] ; can be more than 42000 (return
     * value)
     */
    static void XYZ2Luv(float X, float Y, float Z, float &L, float &u,
                        float &v);
     ...
     * (return value)
     */
    static void Luv2XYZ(float L, float u, float v, float &X, float &Y,
                        float &Z);
    ...
    /**
     * @brief Calculate the effective direction (up or down) to linearly
     * interpolating 2 colors so that it follows the shortest or longest path
     ...
     * @param dir Tells which direction the interpolation have to follow. You
     * can get the value with getHueInterpolationDirection
     * @return The interpolated hue
     */
    static inline double interpolateHueHSV(double h1, double h2, double balance,
                                           eInterpolationDirection dir)
    {
        if (h1 == h2) {
            return h1;
        }
    ...
    /**
     * @brief Interpolate 2 colors from their respective red/green/blue
     * channels, with a balance factor
     ...
     */
    static void interpolateRGBColor(const float balance, const float r1,
                                    const float g1, const float b1,
                                    const float r2, const float g2,
                                    const float b2, int channels,
                                    const double xyz_rgb[3][3],
                                    const double rgb_xyz[3][3], float &ro,
                                    float &go, float &bo);

    /**
    * @brief Interpolate 2 colors from their respective red/green/blue channels,
    with a balance factor
    ...
    * @param bo blue channel of output color [0 ; 65535] (return value)
    */
    static void interpolateRGBColor(
        float realL, float iplow, float iphigh, int algm, const float balance,
        int twoc, int metchrom, float chromat, float luma, const float r1,
        const float g1, const float b1, const float xl, const float yl,
        const float zl, const float x2, const float y2, const float z2,
        const double xyz_rgb[3][3], const double rgb_xyz[3][3], float &ro,
        float &go, float &bo);

    /**
     * @brief Interpolate a hue value as the angle of a polar coordinate with
     * hue in the [0;1] range Chose the shorter path from hue 1 to hue 2.
     * @param h1 First hue [0; 1]
     * @param h2 Second hue  [0; 1]
    ...
    /**
     * @brief Interpolate a hue value as the angle of a polar coordinate with
     * hue in the [-PI ; +PI] range Chose the shorter path from hue 1 to hue 2.
    ...
    static inline double igamma(double x, double gamma, double start,
                                double slope, double mul, double add)
    {
        return (x <= start * slope ? x / slope
                                   : exp(log((x + add) / mul) * gamma));
    }
     ...
     * @return the gamma modified's value [0 ; 1]
     */
    static inline double gamman(double x,
                                double gamma) // standard gamma without slope...
    {
    ...
    static void gammanf2lut(LUTf &gammacurve, float gamma, float divisor,
                            float factor);
    ...
    igamman(double x, double gamma) // standard inverse gamma without slope...
    {
        return exp(log(x) * gamma);
    }
     ...
     * Skin colors: mixed from NX2 skin color palette, Von Luschan, and photos
     * of white, black, yellow people... There are some little exceptions, but
     * it should cover 99% case. Pay attention to white balance, and do not
     * change hue and saturation, upstream of the modification Used by vibrance
     ...
     */
    static void SkinSat(float lum, float hue, float chrom,
                        float &satreduc); // jacques Skin color
    ...
    static void scalered(const float rstprotection, const float param,
                         const float limit, const float HH, const float deltaHH,
                         float &scale, float &scaleext);
    static void transitred(const float HH, const float Chprov1,
                           const float dred, const float factorskin,
                           const float protect_red, const float factorskinext,
                           const float deltaHH, const float factorsat,
                           float &factor);
    static void skinred(double J, double h, double sres, double Sp, float dred,
                        float protect_red, int sk, float rstprotection,
                        float ko, double &s);
    static void skinredfloat(float J, float h, float sres, float Sp, float dred,
                             float protect_red, int sk, float rstprotection,
                             float ko, float &s);
    //  static void scaleredcdbl ( float skinprot, float param, float limit,
    //  float HH, float deltaHH, float &scale,float &scaleext);

    static inline void SkinSatCbdl(float lum, float hue, float chrom,
                                   float skinprot, float &scale, bool neg,
                                   float b_l, float t_l, float t_r)
    {

        static const float C9 = 8.f, C8 = 15.f, C7 = 12.f, C4 = 7.f, C3 = 5.f,
                           C2 = 5.f, C1 = 5.f;
        static const float H9 = 0.05f, H8 = 0.25f, H7 = 0.1f, H4 = 0.02f,
                           H3 = 0.02f, H2 = 0.1f, H1 = 0.1f, H10 = -0.2f,
                           H11 = -0.2f;

        // "real" skin color : take into account a slightly usage of contrast
        // and saturation in RT if option "skin" = 1, uses imolicit factor 1.0
        // wide area  skin color, useful if not accurate colorimetry or if the
        // user has changed hue and saturation, uses explicit facor 0.6 wide
        // area for transition, uses explicit factor 0.4
        ...
        // extended zone for hair, beard and if user adjust high value for
        // skinprot
    ...
    static inline void SkinSatCbdl2(float lum, float hue, float chrom,
                                    float skinprot, float &scale, bool neg,
                                    float b_l, float t_l, float t_r, float b_r,
                                    int basc)
    {
        ...
        // end hue skin algo
                 ...
                 1) { // not hue skin  linear transition or mod chroma curve
            if (hue >= t_l && hue <= t_r) {
                scale = (100.f - skinprot) / 100.1f;
    ...
    static inline void SkinSatCbdlCam(float lum, float hue, float chrom,
                                      float skinprot, float &scale, bool neg,
                                      float b_l, float t_l, float t_r)
    {
    ...
    static void xyz2oklab(float X, float Y, float Z, float &L, float &a,
                          float &b);
    static void oklab2xyz(float L, float a, float b, float &X, float &Y,
                          float &Z);

    template <class T>
    static void rgb2oklab(float R, float G, float B, float &L, float &a,
                          float &b, const T ws[3][3])
    {
        float X, Y, Z;
        rgbxyz(R, G, B, X, Y, Z, ws);
    ...
    template <class T>
    static void oklab2rgb(float L, float a, float b, float &R, float &G,
                          float &B, const T iws[3][3])
    {
        float X, Y, Z;
    ...
    template <class T>
    static void rgb2oklch(float R, float G, float B, float &L, float &c,
                          float &h, const T ws[3][3])
    {
        float a, b;
    ...
    template <class T>
    static void oklch2rgb(float L, float c, float h, float &R, float &G,
                          float &B, const T iws[3][3])
    {
        float a, b;
        jzch2jzazbz(c, h, a, b);
