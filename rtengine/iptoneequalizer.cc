...
void tone_eq(array2D<float> &R, array2D<float> &G, array2D<float> &B,
             const ToneEqualizerParams &pp, const Glib::ustring &workingProfile,
             double scale, bool multithread, bool show_color_map,
             cmsHPROFILE monitor_prof)
{
    const int W = R.width();
...
#pragma omp parallel for if (multithread)
    ...
    for (int y = 0; y < H; ++y) {
        for (int x = 0; x < W; ++x) {
            Y[y][x] = LIM(Color::rgbLuminance(R[y][x], G[y][x], B[y][x], ws),
                          1e-5f, 32.f);
        }
    }
...
#pragma omp parallel for if (multithread)
        ...
        for (int y = 0; y < H; ++y) {
            for (int x = 0; x < W; ++x) {
                float l = LIM(log2(std::max(Y[y][x], 1e-9f)), centers[0],
                              centers[11]);
...
bool ImProcFunctions::toneEqualizer(Imagefloat *rgb)
{
    ...
    const float gain =
        1.f / 65535.f * std::pow(2.f, -params->toneEqualizer.pivot);
    ...
    tone_eq(R, G, B, params->toneEqualizer, params->icm.workingProfile, scale,
            multiThread, show_color_map, monitor);
