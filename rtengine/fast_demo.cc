...
#define CLF 1
        ...
        buffer =
            (char *)calloc(3 * sizeof(float) * TS * TS + 3 * CLF * 64 + 63, 1);
        char *data;
        data = (char *)((uintptr_t(buffer) + uintptr_t(63)) / 64 * 64);

        greentile = (float(*))data; // pointers to array
        redtile =
            (float(*))((char *)greentile + sizeof(float) * TS * TS + CLF * 64);
        bluetile =
            (float(*))((char *)redtile + sizeof(float) * TS * TS + CLF * 64);
...
#pragma omp for nowait
        ...
        for (int top = bord - 2; top < H - bord + 2; top += TS - (4))
            for (int left = bord - 2; left < W - bord + 2; left += TS - (4)) {
                ...
                vmask selmask;
                vmask andmask = _mm_set_epi32(0xffffffff, 0xffffffff,
                ...
                for (int i = top, rr = 0; i < bottom; i++, rr++) {
                    ...
                    for (j = left, cc = 0; j < right - 3; j += 4, cc += 4) {
                        ...
                        absv = vabsf(LVFU(rawData[i - 1][j]) -
                                     LVFU(rawData[i + 1][j]));
                        wtuv = INVGRADV(absv +
                                        vabsf(tempv - LVFU(rawData[i - 2][j])) +
                                        vabsf(LVFU(rawData[i - 1][j]) -
                                              LVFU(rawData[i - 3][j])));
                        wtdv = INVGRADV(absv +
                                        vabsf(tempv - LVFU(rawData[i + 2][j])) +
                                        vabsf(LVFU(rawData[i + 1][j]) -
                                              LVFU(rawData[i + 3][j])));
                        abs2v = vabsf(LVFU(rawData[i][j - 1]) -
                                      LVFU(rawData[i][j + 1]));
                        wtlv = INVGRADV(abs2v +
                                        vabsf(tempv - LVFU(rawData[i][j - 2])) +
                                        vabsf(LVFU(rawData[i][j - 1]) -
                                              LVFU(rawData[i][j - 3])));
                        wtrv = INVGRADV(abs2v +
                                        vabsf(tempv - LVFU(rawData[i][j + 2])) +
                                        vabsf(LVFU(rawData[i][j + 1]) -
                    ...
                    for (; j < right; j++, cc++) {
                        ...
                        if (FC(i, j) == 1) {
                            ...
                            wtu = INVGRAD(
                                (abs(rawData[i + 1][j] - rawData[i - 1][j]) +
                            ...
                            wtd = INVGRAD(
                                (abs(rawData[i - 1][j] - rawData[i + 1][j]) +
                                 ...
                                 abs(rawData[i + 1][j] - rawData[i + 3][j])));
                            wtl = INVGRAD(
                                (abs(rawData[i][j + 1] - rawData[i][j - 1]) +
                            ...
                            // store in rgb array the interpolated G value at
                            // R/B grid points using directional weighted
                            // average
                            greentile[rr * TS + cc] =
                                (wtu * rawData[i - 1][j] +
                                 wtd * rawData[i + 1][j] +
                ...
                for (int i = top + 1, rr = 1; i < bottom - 1; i++, rr++) {
                    if (FC(i, left + (FC(i, 2) & 1) + 1) == 0)
                        ...
                        for (int cc = (FC(i, 2) & 1) + 1, j = left + cc;
                             j < right - 1; j += 2, cc += 2) {
                            // interpolate B/R colors at R/B sites
                            bluetile[rr * TS + cc] =
                                greentile[rr * TS + cc] -
                ...
                for (int i = top + 2, rr = 2; i < bottom - 2; i++, rr++) {
                    ...
                    for (int cc = 2 + (FC(i, 2) & 1), j = left + cc;
                         j < right - 2; j += 4, cc += 4) {
                        // no need to take care about the borders of the tile.
                        ...
                        //              temp2v = greenv -
                        //              zd25v*((LVFU(greentile[(rr-1)*TS+cc])-LVFU(redtile[(rr-1)*TS+cc]))+(LVFU(greentile[(rr+1)*TS+cc])-LVFU(redtile[(rr+1)*TS+cc]))+
                        ...
                        //              temp2v = greenv -
                        //              zd25v*((LVFU(greentile[(rr-1)*TS+cc])-LVFU(bluetile[(rr-1)*TS+cc]))+(LVFU(greentile[(rr+1)*TS+cc])-LVFU(bluetile[(rr+1)*TS+cc]))+
