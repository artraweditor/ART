...
{
    // Load 8 floats from a and combine a[0],a[2],a[4] and a[6] into a vector of
    // 4 floats
...
static INLINE bool vtest(vmask m) { return _mm_movemask_ps((vfloat)m); }
...
// Don't use intrinsics here. Newer gcc versions (>= 4.9, maybe also before 4.9)
// generate better code when not using intrinsics example: vaddf(vmulf(a,b),c)
// will generate an FMA instruction when build for chips with that feature only
...
// Also don't use intrinsic here: Some chips support FMA instructions with 3 and
// 4 operands 3 operands: a = a*b+c, b = a*b+c, c = a*b+c // destination has to
// be one of a,b,c 4 operands: d = a*b+c // destination does not have to be one
// of a,b,c gcc will use the one which fits best when not using intrinsics. With
// using intrinsics that's not possible
...
static INLINE vmask vnotm(vmask x)
{
    return _mm_xor_si128(
        x, _mm_cmpeq_epi32(_mm_setzero_si128(), _mm_setzero_si128()));
}
...
{
    return _mm_and_si128((__m128i)d,
...
static INLINE vdouble visinf(vdouble d)
{
    return (__m128d)_mm_and_si128(
        vmask_isinf(d), _mm_or_si128(vsignbit(d), (__m128i)_mm_set_pd(1, 1)));
}

static INLINE vdouble visinf2(vdouble d, vdouble m)
{
    return (__m128d)_mm_and_si128(vmask_isinf(d),
                                  _mm_or_si128(vsignbit(d), (__m128i)m));
}
...
static INLINE vint vilogbp1(vdouble d)
{
    ...
    __m128i q =
        _mm_and_si128((__m128i)d, _mm_set_epi32(((1 << 12) - 1) << 20, 0,
                                                ((1 << 12) - 1) << 20, 0));
    q = _mm_srli_epi32(q, 20);
    q = vorm(vandm(m, _mm_sub_epi32(
                          q, _mm_set_epi32(300 + 0x3fe, 0, 300 + 0x3fe, 0))),
...
static INLINE vdouble vupper(vdouble d)
{
    return (__m128d)_mm_and_si128(
        (__m128i)d,
        _mm_set_epi32(0xffffffff, 0xf8000000, 0xffffffff, 0xf8000000));
}
...
{
    return dd(
        (__m128d)vorm(vandm(mask, (__m128i)x.x), vandnotm(mask, (__m128i)y.x)),
