...
void boxblur2(const float *const *src, float **dst, float **temp, int startY,
              int startX, int H, int W, int bufferW, int box)
{
...
#pragma omp parallel for
    ...
    for (int row = 0; row < H; ++row) {
        ...
        for (int col = 1; col <= box; ++col, ++len) {
            temp[row][col] = (temp[row][col - 1] * len +
...
#pragma omp for
        ...
        for (int col = 0; col < bufferW - numCols + 1; col += numCols) {
            ...
            for (int row = 1; row <= box; ++row, ++len) {
                for (int n = 0; n < numCols; ++n) {
                    tempvalN[n] =
                        (tempvalN[n] * len + temp[(row + box)][col + n]) /
                        (len + 1);
            ...
            for (int row = box + 1; row < H - box; ++row) {
                for (int n = 0; n < numCols; ++n) {
                    tempvalN[n] += (temp[(row + box)][col + n] -
                                    temp[(row - box - 1)][col + n]) *
                                   rlen;
                    dst[row][col + n] = tempvalN[n];
                }
            }
            ...
            for (int row = H - box; row < H; ++row, --len) {
                for (int n = 0; n < numCols; ++n) {
                    tempvalN[n] = (dst[(row - 1)][col + n] * len -
                                   temp[(row - box - 1)][col + n]) /
                                  (len - 1);
                    dst[row][col + n] = tempvalN[n];
                }
...
void boxblur_resamp(const float *const *src, float **dst, float **temp, int H,
                    int W, int box, int samp)
{
    assert(samp != 0);
...
#pragma omp for
        ...
        for (int row = 0; row < H; ++row) {
            ...
            for (int col = box + 1; col < W - box; ++col) {
                tempval =
                    tempval +
                    (src[row][col + box] - src[row][col - box - 1]) * oneByLen;
...
#pragma omp for nowait
        ...
        for (int col = 0; col < (W / samp) - (numCols - 1); col += numCols) {
            ...
            for (int row = H - box; row < H; ++row, --len) {
                for (int n = 0; n < numCols; ++n) {
                    tempvalN[n] =
                        (tempvalN[n] * len - temp[(row - box - 1)][col + n]) /
                        (len - 1);
                }
...
#pragma omp single
#endif
        {
            ...
            // vertical blur
            for (int col = (W / samp) - ((W / samp) % numCols); col < W / samp;
                 ++col) {
                int len = box + 1;
                ...
                for (int row = 1; row <= box; ++row, ++len) {
                    tempval =
                        (tempval * len + temp[(row + box)][col]) / (len + 1);
                ...
                for (int row = box + 1; row < H - box; ++row) {
                    tempval +=
                        (temp[(row + box)][col] - temp[(row - box - 1)][col]) /
                        len;
                ...
                for (int row = H - box; row < H; ++row, --len) {
                    tempval = (tempval * len - temp[(row - box - 1)][col]) /
                              (len - 1);
...
#define FLT_M(m)                                                               \
    Mat33f(m[0][0], m[0][1], m[0][2], m[1][0], m[1][1], m[1][2], m[2][0],      \
           m[2][1], m[2][2])
...
{
    if (settings->verbose) {
        std::cout << "Applying Highlight Recovery: Color propagation..."
                  << std::endl;
    }
    ...
    if (settings->verbose > 1) {
        for (int c = 0; c < 3; ++c) {
            printf("chmax[%d] : %f\tclmax[%d] : %f\tratio[%d] : %f\n", c,
                   chmax[c], c, clmax[c], c, chmax[c] / clmax[c]);
        }
    }
...
#ifdef _OPENMP
#pragma omp parallel for reduction(min : minx, miny)                           \
    reduction(max : maxx, maxy) schedule(dynamic, 16)
#endif
    ...
    boxblur2(red, channelblur[0], temp, miny, minx, blurHeight, blurWidth,
             bufferWidth, 4);

    boxblur2(green, channelblur[1], temp, miny, minx, blurHeight, blurWidth,
             bufferWidth, 4);

    boxblur2(blue, channelblur[2], temp, miny, minx, blurHeight, blurWidth,
             bufferWidth, 4);
...
#pragma omp parallel for
    ...
    for (int i = 0; i < blurHeight; ++i) {
        for (int j = 0; j < blurWidth; ++j) {
            channelblur[0][i][j] =
                fabsf(channelblur[0][i][j] - red[i + miny][j + minx]) +
                fabsf(channelblur[1][i][j] - green[i + miny][j + minx]) +
                fabsf(channelblur[2][i][j] - blue[i + miny][j + minx]);
        }
    }
    ...
    multi_array2D<float, 4> hilite_full(bufferWidth, blurHeight,
                                        ARRAY2D_CLEAR_DATA, 32);
...
#pragma omp parallel for reduction(+ : hipass_sum, hipass_norm)                \
    ...
    for (int i = 0; i < blurHeight; ++i) {
        for (int j = 0; j < blurWidth; ++j) {
            if ((red[i + miny][j + minx] > thresh[0] ||
                ...
                // if one or more channels is highlight but none are blown, add
                // to highlight accumulator
    ...
    //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    // blur highlight data
    boxblur2(hilite_full[3], hilite_full4, temp, 0, 0, blurHeight, blurWidth,
             bufferWidth, 1);
...
#pragma omp parallel for schedule(dynamic, 16)
    ...
    for (int i = 0; i < blurHeight; ++i) {
        for (int j = 0; j < blurWidth; ++j) {
            if (channelblur[0][i][j] > hipass_ave) {
                // too much variation
                hilite_full[0][i][j] = hilite_full[1][i][j] =
                    hilite_full[2][i][j] = hilite_full[3][i][j] = 0.f;
                continue;
            }

            if (hilite_full4[i][j] > epsilon && hilite_full4[i][j] < 0.95f) {
                // too near an edge, could risk using CA affected pixels,
                // therefore omit
                hilite_full[0][i][j] = hilite_full[1][i][j] =
                    hilite_full[2][i][j] = hilite_full[3][i][j] = 0.f;
            }
    ...
    //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    // blur and resample highlight data; range=size of blur, pitch=sample
    // spacing
    ...
    for (int m = 0; m < 4; ++m) {
        boxblur_resamp(hilite_full[m], hilite[m], temp2, blurHeight, blurWidth,
                       range, pitch);
    ...
    // for faster processing we create two buffers using (height,width) instead
    // of (width,height)
    ...
    for (int j = 1; j < hfw - 1; ++j) {
        for (int i = 2; i < hfh - 2; ++i) {
            ...
            if (hilite[3][i][j] > epsilon) {
                ...
                hilite_dir0[3][j][i] = (hilite_dir0[0 + 3][j - 1][i - 2] +
                                        hilite_dir0[0 + 3][j - 1][i - 1] +
                                        hilite_dir0[0 + 3][j - 1][i] +
                                        hilite_dir0[0 + 3][j - 1][i + 1] +
                                        hilite_dir0[0 + 3][j - 1][i + 2]) == 0.f
                                           ? 0.f
                                           : 0.1f;
            }
        }
...
#pragma omp single
#endif
        {
            ...
            for (int i = 2; i < hfh - 2; ++i) {
                ...
                if (hilite[3][i][hfw - 2] <= epsilon) {
                    hilite_dir[0 + 3][i - 2][hfw - 2] +=
                        hilite_dir4[3][hfw - 2][i];
                    hilite_dir[4 + 3][i + 2][hfw - 2] +=
                        hilite_dir4[3][hfw - 2][i];
                }
            }
        }
...
#pragma omp for nowait
        ...
        for (int c = 0; c < 3; ++c) {
            ...
            for (int i = 2; i < hfh - 2; ++i) {
                ...
                if (hilite[3][i][hfw - 2] <= epsilon) {
                    hilite_dir[0 + c][i - 2][hfw - 2] +=
                        hilite_dir4[c][hfw - 2][i];
                    hilite_dir[4 + c][i + 2][hfw - 2] +=
                        hilite_dir4[c][hfw - 2][i];
                }
            }
        }
...
#endif
        {
            ...
            for (int j = 2; j < hfw - 2; ++j) {
                if (hilite[3][hfh - 2][j] <= epsilon) {
                    hilite_dir[4 + 3][hfh - 1][j] +=
                        hilite_dir[0 + 3][hfh - 2][j];
                }
            }
        }
    }
...
#pragma omp for nowait
        ...
        for (int c = 0; c < 3; ++c) {
            ...
            for (int j = 2; j < hfw - 2; ++j) {
                if (hilite[3][hfh - 2][j] <= epsilon) {
                    hilite_dir[4 + c][hfh - 1][j] +=
                        hilite_dir[0 + c][hfh - 2][j];
                }
            }
        }
...
#pragma omp parallel for
    ...
    for (int dir = 0; dir < 2; ++dir) {
        for (int i = 1; i < hfh - 1; ++i) {
            for (int c = 0; c < 4; ++c) {
                ...
                hilite_dir[dir * 4 + c][i][hfw - 1] =
                    hilite_dir[dir * 4 + c][i][hfw - 2];
            }
        }
        ...
        for (int j = 1; j < hfw - 1; ++j) {
            for (int c = 0; c < 4; ++c) {
                ...
                hilite_dir[dir * 4 + c][hfh - 1][j] =
                    hilite_dir[dir * 4 + c][hfh - 2][j];
            }
        }
        ...
        for (int c = 0; c < 4; ++c) {
            ...
            hilite_dir[dir * 4 + c][0][hfw - 1] =
                hilite_dir[dir * 4 + c][1][hfw - 1] =
                    hilite_dir[dir * 4 + c][0][hfw - 2] =
                        hilite_dir[dir * 4 + c][1][hfw - 2] =
                            hilite_dir[dir * 4 + c][2][hfw - 3];
            hilite_dir[dir * 4 + c][hfh - 1][0] =
                hilite_dir[dir * 4 + c][hfh - 2][0] =
    ...
    for (int c = 0; c < 4; ++c) {
        ...
        hilite_dir0[c][hfw - 1][hfh - 1] = hilite_dir0[c][hfw - 1][hfh - 2] =
            hilite_dir0[c][hfw - 2][hfh - 1] =
                hilite_dir0[c][hfw - 2][hfh - 2] =
    ...
    for (int c = 0; c < 4; ++c) {
        ...
        hilite_dir4[c][hfw - 1][hfh - 1] = hilite_dir4[c][hfw - 1][hfh - 2] =
            hilite_dir4[c][hfw - 2][hfh - 1] =
                hilite_dir4[c][hfw - 2][hfh - 2] =
...
#pragma omp parallel for
        ...
        for (int y = 0; y < H2; ++y) {
            for (int x = 0; x < W2; ++x) {
                guide[y][x] = gamma[Color::rgbLuminance(
                    rbuf[y][x], gbuf[y][x], bbuf[y][x], imatrices.xyz_cam)];
            }
        }
    }
...
#pragma omp parallel for schedule(dynamic, 16)
    ...
    for (int i = 0; i < blurHeight; ++i) {
        ...
        for (int j = 0; j < blurWidth; ++j) {
            const float pixel[3] = {red[i + miny][j + minx],
                                    green[i + miny][j + minx],
                                    blue[i + miny][j + minx]};

            if (pixel[0] < max_f[0] && pixel[1] < max_f[1] &&
                pixel[2] < max_f[2]) {
            ...
            // estimate recovered values using modified HLRecovery_blend
            // algorithm
            float rgb[3] = {pixel[0], pixel[1],
                            pixel[2]}; // Copy input pixel to rgb so it's easier
                                       // to access in loops
            float rgb_blend[3] = {};
            float cam[2][3];
            float lab[2][3];
            ...
            // Initialize cam with raw input [0] and potentially clipped input
            // [1]
            for (int c = 0; c < 3; ++c) {
                cam[0][c] = rgb[c];
            ...
            // first, determine weighted average of unclipped extensions
            // (weighting is by 'hue' proximity)
            bool totwt = false;
            ...
            float Yhi = 1.f / (hilite_dir0[0][j1][i1] + hilite_dir0[1][j1][i1] +
                               hilite_dir0[2][j1][i1]);
            ...
            for (int dir = 0; dir < 2; ++dir) {
                const float Yhi2 = 1.f / (hilite_dir[dir * 4 + 0][i1][j1] +
                                          hilite_dir[dir * 4 + 1][i1][j1] +
            ...
            Yhi = 1.f / (hilite_dir4[0][j1][i1] + hilite_dir4[1][j1][i1] +
                         hilite_dir4[2][j1][i1]);
            ...
            if (pixel[0] > max_f[0] && pixel[1] > max_f[1] &&
                ...
                const float mult =
                    whitept / (0.299f * clipfix[0] + 0.587f * clipfix[1] +
                               0.114f * clipfix[2]);
                ...
                if (notclipped[0] == 0.f) { // red clipped
                    red[yy][xx] = max(
                        pixel[0],
                        clipfix[0] * ((notclipped[1] * pixel[1] +
                                       notclipped[2] * pixel[2]) /
                ...
                if (notclipped[1] == 0.f) { // green clipped
                    green[yy][xx] = max(
                        pixel[1],
                        clipfix[1] * ((notclipped[2] * pixel[2] +
                                       notclipped[0] * pixel[0]) /
                ...
                if (notclipped[2] == 0.f) { // blue clipped
                    blue[yy][xx] = max(
                        pixel[2],
                        clipfix[2] * ((notclipped[0] * pixel[0] +
                                       notclipped[1] * pixel[1]) /
                                      (notclipped[0] * clipfix[0] +
                                       notclipped[1] * clipfix[1] + epsilon)));
                }

                maskval =
                    1.f - (notclipped[0] + notclipped[1] + notclipped[2]) / 5.f;
            }

            Y = 0.299f * red[yy][xx] + 0.587f * green[yy][xx] +
                0.114f * blue[yy][xx];
    ...
    if (blur > 0) {
        ...
        if (blur > 0) { // no use of 2nd guidedFilter if Blur = 0 (slider to
                        // 1)..speed-up and very small differences.
        ...
        for (int y = 0; y < blurHeight; ++y) {
            ...
            for (int x = 0; x < blurWidth; ++x) {
                ...
                if (m > 0.f) {
                    ...
                    red[y + miny][x + minx] =
                        intp(m, getBilinearValue(rbuf, fx, fy),
                             red[y + miny][x + minx]);
                    green[y + miny][x + minx] =
                        intp(m, getBilinearValue(gbuf, fx, fy),
                             green[y + miny][x + minx]);
                    blue[y + miny][x + minx] =
                        intp(m, getBilinearValue(bbuf, fx, fy),
                             blue[y + miny][x + minx]);
                }
            }
        }
...
/* The refavg values are calculated in raw-RGB-cube3 space
   We calculate all color channels in the 3x3 photosite area, this can be
   understaood as a "superpixel", the "asking" location is in the centre. As
   this works for bayer and xtrans sensors we don't have a fixed ratio but
   calculate the average for every color channel first. refavg for one of red,
   green or blue is defined as means of both other color channels (opposing).

   The basic idea / observation for the _process_opposed algorithm is, the
   refavg is a good estimate for any clipped color channel in the vast majority
   of images, working mostly fine both for small specular highlighted spots and
   large areas.

   The correction via some sort of global chrominance further helps to correct
   color casts. The chrominace data are taken from the areas morphologically
   very close to clipped data. Failures of the algorithm (color casts) are in
   most cases related to a) very large differences between optimal white balance
   coefficients vs what we have as D65 in the darktable pipeline b) complicated
   lightings so the gradients are not well related c) a wrong whitepoint setting
   in the rawprepare module. d) the maths might not be best
*/
//-----------------------------------------------------------------------------
...
{
    int retval = 0;
    retval = img[i - w1 - 1] | img[i - w1] | img[i - w1 + 1] | img[i - 1] |
             img[i] | img[i + 1] | img[i + w1 - 1] | img[i + w1] |
             img[i + w1 + 1];
    if (retval) {
        return retval;
    }
    ...
    retval = img[i - w3 - 2] | img[i - w3 - 1] | img[i - w3] | img[i - w3 + 1] |
             img[i - w3 + 2] | img[i - w2 - 3] | img[i - w2 - 2] |
             img[i - w2 + 2] | img[i - w2 + 3] | img[i - w1 - 3] |
             img[i - w1 + 3] | img[i - 3] | img[i + 3] | img[i + w1 - 3] |
             img[i + w1 + 3] | img[i + w2 - 3] | img[i + w2 - 2] |
             img[i + w2 + 2] | img[i + w2 + 3] | img[i + w3 - 2] |
...
#endif
    for (int row = HL_BORDER; row < height - HL_BORDER; row++) {
        for (int col = HL_BORDER, i = row * w1 + col; col < w1 - HL_BORDER;
             col++, i++) {
...
{
    if (settings->verbose) {
        std::cout << "Applying Highlight Recovery: Inpaint opposed..."
                  << std::endl;
    }
    ...
    const float clips[3] = {clipval * float(rr), clipval * float(gg),
                            clipval * float(bb)};
    const float clipdark[3] = {0.03f * clips[0], 0.125f * clips[1],
                               0.03f * clips[2]};

    bool anyclipped = false;
    float **chan[3] = {red, green, blue};

    const float clipscale[3] = {clips[0] / scalecoeffs[0],
...
#pragma omp parallel for
    ...
    const auto mask_val = [&](int c, int y, int x) -> int & {
        return mask_buffer[c * p_size + (HL_BORDER + y) * pwidth + x +
                           HL_BORDER];
    };
    ...
    for (int y = 1; y < cH - 1; ++y) {
        ...
        for (int x = 1; x < cW - 1; ++x) {
            ...
            for (int c = 0; c < 3; ++c) {
                ...
                if (mask_val(c, y, x) && (inval > clipdark[c]) &&
                    (inval < clips[c])) {
    ...
    for (int y = 0; y < cH; ++y) {
        ...
        for (int x = 0; x < cW; ++x) {
            ...
            for (int c = 0; c < 3; ++c) {
                ...
                if (inval >= clips[c]) {
                    chan[c][yy][xx] =
                        std::max(inval, tmp[c][y][x] + chrominance[c]);
                }
            }
        }
    }
