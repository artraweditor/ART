...
void RawImageSource::ahd_demosaic()
{
    ...
    for (int i = 0; i < 3; i++)
        for (unsigned int j = 0; j < 3; j++) {
            ...
            for (int k = 0; k < 3; k++) {
                xyz_cam[i][j] +=
                    xyz_rgb[i][k] * imatrices.rgb_cam[k][j] / d65_white[i];
            }
        }
...
#pragma omp for collapse(2) schedule(dynamic) nowait
        ...
        for (int top = 2; top < height - 5; top += TS - 6) {
            for (int left = 2; left < width - 5; left += TS - 6) {
                ...
                for (int d = 0; d < 2; d++)
                    for (int row = top + 1;
                         row < top + TS - 1 && row < height - 3; row++) {
                        int cng = FC(row + 1, FC(row + 1, 0) & 1);
                        for (int col = left + 1;
                             col < std::min(left + TS - 1, width - 3); col++) {
                            auto pix = &rawData[row][col];
                            auto rix = &rgb[d][row - top][col - left];
                            auto lix = lab[d][row - top][col - left];
                            if (FC(row, col) == 1) {
                                rix[0][2 - cng] = CLIP(
                                    pix[0] + (0.5f * (pix[-1] + pix[1] -
                                                      rix[-1][1] - rix[1][1])));
                                rix[0][cng] =
                                    CLIP(pix[0] +
                                         (0.5f * (pix[-width] + pix[width] -
                                                  rix[-TS][1] - rix[TS][1])));
                                rix[0][1] = pix[0];
                            } else {
                                rix[0][cng] =
                ...
                for (int row = top + 2; row < top + TS - 2 && row < height - 4;
                     row++) {
                    int tr = row - top;
                    ...
                    for (int col = left + 2, tc = 2;
                        ...
                        float leps =
                            std::min(std::max(ldiff[0][0], ldiff[0][1]),
                                     std::max(ldiff[1][2], ldiff[1][3]));
                        float abeps =
                            std::min(std::max(abdiff[0][0], abdiff[0][1]),
                                     std::max(abdiff[1][2], abdiff[1][3]));

                        for (int d = 0; d < 2; d++) {
                            homo[d][tr][tc] = 0;
                            for (int i = 0; i < 4; i++) {
                                homo[d][tr][tc] += (ldiff[d][i] <= leps) *
                                                   (abdiff[d][i] <= abeps);
                            }
                        }
                    }
                ...
                for (int row = top + 3; row < top + TS - 3 && row < height - 5;
                     row++) {
                    int tr = row - top;

                    for (int col = left + 3, tc = 3;
                         col < std::min(left + TS - 3, width - 5);
                         col++, tc++) {
