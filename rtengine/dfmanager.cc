...
std::string DFInfo::key(const std::string &mak, const std::string &mod, int iso,
                        double shut)
{
    std::ostringstream s;
    s << mak << " " << mod << " ";
...
double DFInfo::distance(const std::string &mak, const std::string &mod, int iso,
...
/* updateRawImage() load into ri the actual pixel data from pathname if there is
 * a single shot otherwise load each file from the pathNames list and extract a
 * template from the media; the first file is used also for reading all
 * information other than pixels
 */
void DFInfo::updateRawImage()
{
    ...
    if (!pathNames.empty()) {
        ...
        ri = new RawImage(
            *iName); // First file used also for extra pixels information
        ...
        if (ri->loadRaw(true)) {
            ...
            int W = ri->get_width();
            ri->compress_image(0);
            int rSize = W * ((ri->getSensorType() == ST_BAYER ||
                ...
                if (!temp->loadRaw(true)) {
                    ...
                    if (ri->getSensorType() == ST_BAYER ||
                        ...
                        for (int row = 0; row < H; row++) {
                            for (int col = 0; col < W; col++) {
                                acc[row][3 * col + 0] +=
                                    temp->data[row][3 * col + 0];
                                acc[row][3 * col + 1] +=
                                    temp->data[row][3 * col + 1];
                                acc[row][3 * col + 2] +=
                                    temp->data[row][3 * col + 2];
                            }
                        }
                    }
                }
...
void DFInfo::updateBadPixelList(RawImage *df)
{
    ...
    if (df->getSensorType() == ST_BAYER ||
        df->getSensorType() == ST_FUJI_XTRANS) {
...
#pragma omp for nowait
            ...
            for (int row = 2; row < df->get_height() - 2; row++)
                for (int col = 2; col < df->get_width() - 2; col++) {
                    float m =
                        (df->data[row - 2][col - 2] + df->data[row - 2][col] +
...
#pragma omp critical
        ...
        badPixels.insert(badPixels.end(), badPixelsTemp.begin(),
                         badPixelsTemp.end());
    } else {
    ...
    if (settings->verbose) {
        std::cout << "Extracted " << badPixels.size()
                  << " pixels from darkframe:" << df->get_filename().c_str()
                  << std::endl;
    }
}
...
void DFManager::init(const Glib::ustring &pathname)
{
    ...
    try {
        ...
        while (auto file = enumerator->next_file()) {
            names.emplace_back(
                Glib::build_filename(pathname, file->get_name()));
        }
    ...
    for (size_t i = 0; i < names.size(); i++) {
        ...
        if (lastdot != Glib::ustring::npos &&
            names[i].substr(lastdot) == ".badpixels") {
    ...
    for (dfList_t::iterator iter = dfList.begin(); iter != dfList.end();
         ++iter) {
        ...
        if (settings->verbose) {
            if (!i.pathname.empty()) {
                ...
                for (std::list<Glib::ustring>::iterator iter =
                         i.pathNames.begin();
                     iter != i.pathNames.end(); ++iter) {
                    printf("%s, ", iter->c_str());
                }
...
DFInfo *DFManager::addFileInfo(const Glib::ustring &filename, bool pool)
{
    ...
    try {
        ...
        /* Files are added in the map, divided by same maker/model,ISO and
         * shutter*/
        std::string key(
            DFInfo::key(((Glib::ustring)idata.getMake()).uppercase(),
                        ((Glib::ustring)idata.getModel()).uppercase(),
        ...
        if (iter == dfList.end()) {
            ...
            while (iter != dfList.end() && iter->second.key() == key &&
                   ABS(iter->second.timestamp - idata.getDateTimeAsTS()) >
                       60 * 60 * 6) { // 6 hour difference
                ++iter;
            }
 ...
 *  if perfect matches for iso and shutter are found, then the list is scanned
 * for lesser distance in time otherwise if no match is found, the whole list is
 * searched for lesser distance in iso and shutter
 */
DFInfo *DFManager::find(const std::string &mak, const std::string &mod,
                        int isospeed, double shut, time_t t)
{
    if (dfList.empty()) {
        return nullptr;
    }
    ...
    if (iter != dfList.end()) {
        ...
        for (++iter; iter != dfList.end() && !key.compare(iter->second.key());
             ++iter) {
...
RawImage *DFManager::searchDarkFrame(const std::string &mak,
                                     const std::string &mod, int iso,
                                     double shut, time_t t)
{
    DFInfo *df = find(((Glib::ustring)mak).uppercase(),
                      ((Glib::ustring)mod).uppercase(), iso, shut, t);
...
std::vector<badPix> *DFManager::getHotPixels(const std::string &mak,
                                             const std::string &mod, int iso,
                                             double shut, time_t t)
{
    DFInfo *df = find(((Glib::ustring)mak).uppercase(),
...
int DFManager::scanBadPixelsFile(Glib::ustring filename)
{
    ...
    if (fgets(line, sizeof(line), file)) {
        ...
        if (numparms == 1) { // only one number in first line means, that this
                             // is the offset.
            offset = x;
        } else if (numparms == 2) {
...
std::vector<badPix> *DFManager::getBadPixels(const std::string &mak,
