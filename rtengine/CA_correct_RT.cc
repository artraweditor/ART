...
//  copyright (c) for improvements (speedups, iterated correction and avoid
//  colour shift) 2018 Ingo Weyrich <heckflosse67@gmx.de>
//
...
bool LinEqSolve(int nDim, double *pfMatr, double *pfVect, double *pfSolution)
{
    ...
    for (k = 0; k < (nDim - 1); k++) { // base row of matrix
        ...
        for (j = (k + 1); j < nDim; j++) { // current row of matrix
            ...
            pfVect[j] =
                pfVect[j] + fAcc * pfVect[k]; // free member recalculation
        }
    }
                                     ...
                                     float *buffer, bool freeBuffer)
{
    ...
    if (measure) {
        std::cout << "CA correcting " << W << "x" << H << " image with "
    ...
    constexpr int v1 = ts, v2 = 2 * ts, v3 = 3 * ts,
                  v4 = 4 * ts; //, p1=-ts+1, p2=-2*ts+2, p3=-3*ts+3, m1=ts+1,
                               //m2=2*ts+2, m3=3*ts+3;
    ...
    if (avoidColourshift) {
        redFactor =
            new array2D<float>((W + 1 - 2 * cb) / 2, (H + 1 - 2 * cb) / 2);
        blueFactor =
            new array2D<float>((W + 1 - 2 * cb) / 2, (H + 1 - 2 * cb) / 2);
...
#pragma omp parallel for
    ...
    const int vblsz =
        ceil((float)(height + border2) / (ts - border2) + 2 + vz1);
    const int hblsz = ceil((float)(width + border2) / (ts - border2) + 2 + hz1);

    // temporary array to store simple interpolation of G
    if (!buffer) {
        buffer = static_cast<float *>(malloc(
            (height * width + vblsz * hblsz * (2 * 2 + 1)) * sizeof(float)));
    }
    float *Gtmp = buffer;
    ...
    memset(blockwt, 0,
           static_cast<unsigned long>(vblsz) * hblsz * (2 * 2 + 1) *
               sizeof(float));
    float(*blockshifts)[2][2] = (float(*)[2][2])(blockwt + vblsz * hblsz);

    // Because we can't break parallel processing, we need a switch do handle
    // the errors
    ...
    const bool fitParamsSet =
        fitParamsTransfer && fitParamsIn && iterations < 2;
    ...
    for (size_t it = 0; it < iterations && processpasstwo; ++it) {
        ...
        constexpr float eps = 1e-5f,
                        eps2 = 1e-10f; // tolerance to avoid dividing by zero
...
#pragma omp parallel
#endif
        {
            ...
            constexpr int buffersize = sizeof(float) * ts * ts +
                                       8 * sizeof(float) * ts * tsh + 8 * 64 +
                                       63;
            constexpr int buffersizePassTwo = sizeof(float) * ts * ts +
                                              4 * sizeof(float) * ts * tsh +
                                              4 * 64 + 63;
            char *const bufferThr = (char *)malloc(
                (autoCA && !fitParamsSet) ? buffersize : buffersizePassTwo);

            char *const data =
                (char *)((uintptr_t(bufferThr) + uintptr_t(63)) / 64 * 64);

            // shift the beginning of all arrays but the first by 64 bytes to
            // avoid cache miss conflicts on CPUs which have <= 4-way
            // associative L1-Cache
            ...
            if (autoCA && !fitParamsSet) {
                ...
                float *rblpfh =
                    (float *)(data + 3 * sizeof(float) * ts * ts + 5 * 64);
                // low pass filter for R/B in vertical direction
                float *rblpfv = (float *)(data + 3 * sizeof(float) * ts * ts +
                                          sizeof(float) * ts * tsh + 6 * 64);
                // low pass filter for colour differences in horizontal
                // direction
                float *grblpfh =
                    (float *)(data + 4 * sizeof(float) * ts * ts + 7 * 64);
                ...
                // Main algorithm: Tile loop calculating correction parameters
                // per tile
...
#pragma omp for collapse(2) schedule(dynamic, chunkSize) nowait
                ...
                for (int top = -border; top < height; top += ts - border2) {
                    for (int left = -border; left < width - (W & 1);
                         left += ts - border2) {
                        ...
                        const int ccmax = (right > width - (W & 1))
                                              ? width - (W & 1) - left
                                              : cc1;
                        ...
                        for (int rr = rrmin; rr < rrmax; rr++) {
                            ...
                            for (; cc < ccmax - 7;
                                ...
                                vfloat val2 =
                                    LVFU(rawData[row][col + 4]) / c65535v;
                                vfloat nonGreenv = _mm_shuffle_ps(
                                    val1, val2, _MM_SHUFFLE(2, 0, 2, 0));
                        ...
                        for (int rr = 3; rr < rr1 - 3; rr++) {
                            ...
                            for (; cc < cc1 - 9; cc += 8, indx += 8) {
                                ...
                                // store in rgb array the interpolated G value
                                // at R/B grid points using directional weighted
                                // average
                                vfloat result =
                                    (wtuv * rgb1mv1v + wtdv * rgb1pv1v +
                        ...
                        for (int rr = 4; rr < rr1 - 4; rr++) {
                            ...
                            for (; cc < cc1 - 10; cc += 8, indx += 8) {
                                ...
                                rgb1v = vmul2f(rgb1v);
                                vfloat glpfvv =
                                    (rgb1v + LC2VFU(rgb[1][indx + v2]) +
                                     LC2VFU(rgb[1][indx - v2]));
                                vfloat glpfhv =
                                    (rgb1v + LC2VFU(rgb[1][indx + 2]) +
                                     LC2VFU(rgb[1][indx - 2]));
                                rgbcv = vmul2f(rgbcv);
                                STVFU(
                                    rblpfv[indx >> 1],
                            ...
                            for (; cc < cc1 - 4; cc += 2, indx += 2) {
                                ...
                                // vertical/horizontal directions
                                float glpfv =
                                    (2.f * rgb[1][indx] + rgb[1][indx + v2] +
                                     rgb[1][indx - v2]);
                                float glpfh =
                                    (2.f * rgb[1][indx] + rgb[1][indx + 2] +
                                     rgb[1][indx - 2]);
                                rblpfv[indx >> 1] =
                                    0.25f *
                                    fabsf(glpfv - (2.f * rgb[c][indx >> 1] +
                                                   rgb[c][(indx + v2) >> 1] +
                                                   rgb[c][(indx - v2) >> 1]));
                                rblpfh[indx >> 1] =
                                    0.25f *
                                    fabsf(glpfh - (2.f * rgb[c][indx >> 1] +
                                                   rgb[c][(indx + 2) >> 1] +
                                                   rgb[c][(indx - 2) >> 1]));
                                grblpfv[indx >> 1] =
                                    0.25f *
                                    (glpfv + (2.f * rgb[c][indx >> 1] +
                                              rgb[c][(indx + v2) >> 1] +
                                              rgb[c][(indx - v2) >> 1]));
                                grblpfh[indx >> 1] =
                                    0.25f * (glpfh + (2.f * rgb[c][indx >> 1] +
                                                      rgb[c][(indx + 2) >> 1] +
                                                      rgb[c][(indx - 2) >> 1]));
                            }
                        }
                        ...
                        // along line segments, find the point along each
                        // segment that minimizes the colour variance averaged
                        // over the tile; evaluate for up/down and left/right
                        // away from R/B grid point
                        for (int rr = 8; rr < rr1 - 8; rr++) {
                            ...
                            for (; cc < cc1 - 14; cc += 8, indx += 8) {
                                // in linear interpolation, colour differences
                                // are a quadratic function of interpolation
                                // position; solve for the interpolation
                                // position that minimizes colour difference
                                // variance over the tile

                                // vertical
                                vfloat temp1 =
                                    zd3v * (LC2VFU(rgb[1][indx + ts + 1]) -
                                ...
                                vfloat gdiffvv = (LC2VFU(rgb[1][indx + ts]) -
                                                 ...
                                                 (temp1 - temp2);
                                vfloat deltgrbv = LVFU(rgb[c][indx >> 1]) -
                                                  LC2VFU(rgb[1][indx]);

                                vfloat gradwtvv =
                                    (LVFU(rbhpfv[indx >> 1]) +
                                     zd5v * (LVFU(rbhpfv[(indx >> 1) + 1]) +
                                ...
                                // horizontal
                                vfloat gdiffhv = (LC2VFU(rgb[1][indx + 1]) -
                                                  LC2VFU(rgb[1][indx - 1])) +
                                                 (temp1 + temp2);

                                vfloat gradwthv =
                                    (LVFU(rbhpfh[indx >> 1]) +
                                     zd5v * (LVFU(rbhpfh[(indx >> 1) + v1]) +
                                             LVFU(rbhpfh[(indx >> 1) - v1]))) *
                                    (LVFU(grblpfh[(indx >> 1) - 1]) +
                                     LVFU(grblpfh[(indx >> 1) + 1])) /
                                    (epsv +
                                     zd1v * (LVFU(grblpfh[(indx >> 1) - 1]) +
                                             LVFU(grblpfh[(indx >> 1) + 1])) +
                            ...
                            for (; cc < cc1 - 8; cc += 2, indx += 2) {
                                ...
                                float gdiff =
                                    ...
                                    0.3f * (rgb[1][indx + ts + 1] -
                                            rgb[1][indx - ts + 1] +
                                            rgb[1][indx + ts - 1] -
                                            rgb[1][indx - ts - 1]);
                                float deltgrb =
                                    (rgb[c][indx >> 1] - rgb[1][indx]);

                                float gradwt =
                                    (rbhpfv[indx >> 1] +
                                     0.5f * (rbhpfv[(indx >> 1) + 1] +
                                             rbhpfv[(indx >> 1) - 1])) *
                                    (grblpfv[(indx >> 1) - v1] +
                                     grblpfv[(indx >> 1) + v1]) /
                                    (eps +
                                ...
                                coeff[0][0][c >> 1] +=
                                    gradwt * deltgrb * deltgrb;
                                coeff[0][1][c >> 1] += gradwt * gdiff * deltgrb;
                                coeff[0][2][c >> 1] += gradwt * gdiff * gdiff;

                                // horizontal
                                gdiff = (rgb[1][indx + 1] - rgb[1][indx - 1]) +
                                        0.3f * (rgb[1][indx + 1 + ts] -
                                                rgb[1][indx - 1 + ts] +
                                                rgb[1][indx + 1 - ts] -
                                                rgb[1][indx - 1 - ts]);

                                gradwt = (rbhpfh[indx >> 1] +
                                          0.5f * (rbhpfh[(indx >> 1) + v1] +
                                                  rbhpfh[(indx >> 1) - v1])) *
                                         (grblpfh[(indx >> 1) - 1] +
                                          grblpfh[(indx >> 1) + 1]) /
                                         (eps +
                                          0.1f * (grblpfh[(indx >> 1) - 1] +
                                                  grblpfh[(indx >> 1) + 1]) +
                                          rblpfh[(indx >> 1) - 1] +
                                          rblpfh[(indx >> 1) + 1]);
                        ...
                        for (int c = 0; c < 2; c++) {
                            for (int dir = 0; dir < 2; dir++) { // vert/hor
                                ...
                                // This is the approximate _optical_ location of
                                // the R/B pixels
                                if (coeff[dir][2][c] > eps2) {
                                    CAshift[dir][c] =
                                        coeff[dir][1][c] / coeff[dir][2][c];
                                ...
                                // offset gives NW corner of square containing
                                // the min; dir : 0=vert, 1=hor
                                ...
                                // evaluate the shifts to the location that
                                // minimizes CA within the tile
                                blockshifts[vblock * hblsz + hblock][c][dir] =
                                    CAshift[dir]
                                           [c]; // vert/hor CA shift for R/B
                            } // vert/hor
                        } // colour
...
#pragma omp single
#endif
                {
                    ...
                    if (processpasstwo) {
                        for (int vblock = 1; vblock < vblsz - 1;
                             vblock++) { // left and right sides
                            for (int c = 0; c < 2; c++) {
                                for (int i = 0; i < 2; i++) {
                                    blockshifts[vblock * hblsz][c][i] =
                                        blockshifts[(vblock)*hblsz + 2][c][i];
                                    blockshifts[vblock * hblsz + hblsz -
                                                1][c][i] =
                                        blockshifts[(vblock)*hblsz + hblsz - 3]
                                                   [c][i];
                                }
                            }
                        }

                        for (int hblock = 0; hblock < hblsz;
                             hblock++) { // top and bottom sides
                            for (int c = 0; c < 2; c++) {
                                for (int i = 0; i < 2; i++) {
                                    blockshifts[hblock][c][i] =
                                        blockshifts[2 * hblsz + hblock][c][i];
                                    blockshifts[(vblsz - 1) * hblsz +
                                                hblock][c][i] =
                                        blockshifts[(vblsz - 3) * hblsz +
                                                    hblock][c][i];
                                }
                            }
                        }
                        ...
                        for (int i = 0; i < 256; i++) {
                            polymat[0][0][i] = polymat[0][1][i] =
                                polymat[1][0][i] = polymat[1][1][i] = 0;
                        }

                        for (int i = 0; i < 16; i++) {
                            shiftmat[0][0][i] = shiftmat[0][1][i] =
                                shiftmat[1][0][i] = shiftmat[1][1][i] = 0;
                        }
                        ...
                        for (int vblock = 1; vblock < vblsz - 1; vblock++) {
                            for (int hblock = 1; hblock < hblsz - 1; hblock++) {
                                // block 3x3 median of blockshifts for
                                // robustness
                                for (int c = 0; c < 2; c++) {
                                    float bstemp[2];
                                    for (int dir = 0; dir < 2; dir++) {
                                        ...
                                        const std::array<float, 9> p = {
                                            blockshifts[(vblock - 1) * hblsz +
                                                        hblock - 1][c][dir],
                                            blockshifts[(vblock - 1) * hblsz +
                                                        hblock][c][dir],
                                            blockshifts[(vblock - 1) * hblsz +
                                                        hblock + 1][c][dir],
                                            blockshifts[(vblock)*hblsz +
                                                        hblock - 1][c][dir],
                                            blockshifts[(vblock)*hblsz + hblock]
                                                       [c][dir],
                                            blockshifts[(vblock)*hblsz +
                                                        hblock + 1][c][dir],
                                            blockshifts[(vblock + 1) * hblsz +
                                    ...
                                    // now prepare coefficient matrix; use only
                                    // data points within caAutostrength/2 std
                                    // devs of zero
                                    if (SQR(bstemp[0]) >
                                            caAutostrength * blockvar[0][c] ||
                                        SQR(bstemp[1]) >
                                    ...
                                    for (int dir = 0; dir < 2; dir++) {
                                        ...
                                        for (int i = 0; i < polyord; i++) {
                                            ...
                                            for (int j = 0; j < polyord; j++) {
                                                double powVblock =
                                                    powVblockInit;
                                                for (int m = 0; m < polyord;
                                                     m++) {
                                                    double powHblock =
                                                        powHblockInit;
                                                    for (int n = 0; n < polyord;
                                                         n++) {
                                                        polymat[c][dir]
                                                               [numpar *
                                                                    (polyord *
                        ...
                        // if too few data points, restrict the order of the fit
                        // to linear
                        if (numblox[1] < 32) {
                            polyord = 2;
                            numpar = 4;
                            ...
                            for (int c = 0; c < 2; c++) {
                                for (int dir = 0; dir < 2; dir++) {
                                    if (!LinEqSolve(numpar, polymat[c][dir],
                                                    ...
                                                    fitparams[c][dir])) {
                                        std::cout
                                            << "CA correction pass failed -- "
                                               "can't solve linear equations "
                                               "for colour %d direction "
                                            << c << std::endl;
                                        processpasstwo = false;
                                    }
                                }
                    ...
                    // fitparams[polyord*i+j] gives the coefficients of
                    // (vblock^i hblock^j) in a polynomial fit for i,j<=4
                }
...
#pragma omp for schedule(dynamic, chunkSize) collapse(2)
                ...
                for (int top = -border; top < height; top += ts - border2) {
                    for (int left = -border; left < width - (W & 1);
                        ...
                        if (rrmax < rr1) {
                            for (int rr = 0; rr < std::min(border, rr1 - rrmax);
                                 rr++) {
                                for (int cc = ccmin; cc < ccmax; cc++) {
                                    int c = fc(cfa, rr, cc);
                                    rgb[c][((rrmax + rr) * ts + cc) >>
                        ...
                        if (ccmax < cc1) {
                            for (int rr = rrmin; rr < rrmax; rr++) {
                                for (int cc = 0;
                                     cc < std::min(border, cc1 - ccmax); cc++) {
                                    int c = fc(cfa, rr, cc);
                                    rgb[c][(rr * ts + ccmax + cc) >>
                                           ((c & 1) ^ 1)] =
                                        (rawData[(top + rr)]
...
#endif
                            // manual CA correction; use red/blue slider values
                            // to set CA shift parameters
                            for (int rr = 3; rr < rr1 - 3; rr++) {
                                ...
                                for (; cc < cc1 - 10; cc += 8, indx += 8) {
                                    ...
                                    vfloat val1v =
                                        ...
                                        vabsf(
                                            ...
                                            LC2VFU(rgb[1][(rr - 1) * ts + cc]));
                                    vfloat val2v =
                                        epsv + vabsf(LC2VFU(rgb[1][indx + 1]) -
                                                     LC2VFU(rgb[1][indx - 1]));
                                    vfloat wtuv =
                                        onev /
                                        SQRV(val1v +
                                             vabsf(LVFU(rgb[c][(rr * ts + cc) >>
                                                               1]) -
                                                   LVFU(rgb[c][((rr - 2) * ts +
                                                                cc) >>
                                                               1])) +
                                             vabsf(LC2VFU(rgb[1][(rr - 1) * ts +
                                                                 cc]) -
                                                   LC2VFU(rgb[1][(rr - 3) * ts +
                                                                 cc])));
                                    vfloat wtdv =
                                        onev /
                                        SQRV(val1v +
                                    ...
                                    // store in rgb array the interpolated G
                                    // value at R/B grid points using
                                    // directional weighted average
                                    vfloat result =
                                        (wtuv * LC2VFU(rgb[1][indx - v1]) +
                        ...
                        if (!autoCA) {
                            float hfrac =
                                -((float)(hblock - 0.5) / (hblsz - 2) - 0.5);
                            float vfrac =
                                -((float)(vblock - 0.5) / (vblsz - 2) - 0.5) *
                                height / width;
                            ...
                            // CA auto correction; use CA diagnostic pass to set
                            // shift parameters
                            ...
                            for (int i = 0; i < polyord; i++) {
                                ...
                                for (int j = 0; j < polyord; j++) {
                                    ...
                                    lblockshifts[1][0] +=
                                        powHblock *
                                        fitparams[1][0][polyord * i + j];
                                    lblockshifts[1][1] +=
                                        powHblock *
                                        fitparams[1][1][polyord * i + j];
                                    powHblock *= hblock;
                                }
                                powVblock *= vblock;
                            }
                            constexpr float bslim = 3.99; // max allowed CA
                                                          // shift
                            lblockshifts[0][0] =
                        ...
                        for (int c = 0; c < 3; c += 2) {
                            ...
                            shiftvfloor[c] =
                                floor((float)lblockshifts[c >> 1][0]);
                            shiftvceil[c] =
                                ceil((float)lblockshifts[c >> 1][0]);
                            ...
                            shifthfloor[c] =
                                floor((float)lblockshifts[c >> 1][1]);
                            shifthceil[c] =
                                ceil((float)lblockshifts[c >> 1][1]);
                        ...
                        for (int rr = 4; rr < rr1 - 4; rr++) {
                            ...
                            int indx = (rr * ts + cc) >> 1;
                            int indxfc =
                                (rr + shiftvfloor[c]) * ts + cc + shifthceil[c];
                            int indxff = (rr + shiftvfloor[c]) * ts + cc +
                                         shifthfloor[c];
                            int indxcc =
                            ...
                            for (; cc < cc1 - 10; cc += 8, indxfc += 8,
                                                  indxff += 8, indxcc += 8,
                                                  indxcf += 8, indx += 4) {
                                // perform CA correction using colour ratios or
                                // colour differences
                                vfloat Ginthfloorv =
                                ...
                                vfloat Ginthceilv =
                                    vintpf(shifthfracv, LC2VFU(rgb[1][indxcc]),
                                           LC2VFU(rgb[1][indxcf]));
                                // Gint is bilinear interpolation of G at CA
                                // shift point
                                vfloat Gintv = vintpf(shiftvfracv, Ginthceilv,
                                                      Ginthfloorv);

                                // determine R/B at grid points using colour
                                // differences at shift point plus interpolated
                                // G value at grid point but first we need to
                                // interpolate G-R/G-B to grid points...
                                STVFU(grbdiff[indx],
                                      Gintv - LVFU(rgb[c][indx]));
                                STVFU(gshift[indx], Gintv);
                            }

#endif
                            for (; cc < cc1 - 4; cc += 2, indxfc += 2,
                                                 indxff += 2, indxcc += 2,
                                                 indxcf += 2, ++indx) {
                                // perform CA correction using colour ratios or
                                // colour differences
                                float Ginthfloor =
                                    intp(shifthfrac[c], rgb[1][indxfc],
                                         rgb[1][indxff]);
                                float Ginthceil =
                        ...
                        for (int rr = 8; rr < rr1 - 8; rr++) {
                            ...
                            for (int indx = rr * ts + cc; cc < cc1 - 14;
                                 cc += 8, indx += 8) {
                                // interpolate colour difference from optical
                                // R/B locations to grid locations
                                vfloat grbdiffinthfloor =
                                    vintpf(shifthfracc,
                                           LVFU(grbdiff[(indx - GRBdir1) >> 1]),
                                           LVFU(grbdiff[indx >> 1]));
                                vfloat grbdiffinthceil = vintpf(
                                    ...
                                    LVFU(grbdiff[((rr - GRBdir0) * ts + cc) >>
                                                 1]));
                                // grbdiffint is bilinear interpolation of
                                // G-R/G-B at grid point
                                vfloat grbdiffint =
                                    vintpf(shiftvfracc, grbdiffinthceil,
                                           grbdiffinthfloor);

                                // now determine R/B at grid points using
                                // interpolated colour differences and
                                // interpolated G value at grid point
                                ...
                                if (_mm_movemask_ps((vfloat)cmask)) {
                                    // if for any of the 4 pixels the condition
                                    // is true, do the math for all 4 pixels and
                                    // mask the unused out at the end
                                    // gradient weights using difference from G
                                    // at CA shift points and G at grid points
                                    vfloat p0 =
                                        onev /
                                        (epsv +
                                         vabsf(rinv - LVFU(gshift[indx >> 1])));
                                    vfloat p1 =
                                        onev /
                                        (epsv +
                                         vabsf(rinv -
                                               LVFU(gshift[(indx - GRBdir1) >>
                                                           1])));
                                    vfloat p2 =
                                        onev /
                                        (epsv +
                                    ...
                                    grbdiffint = vself(
                                        ...
                                        (p0 * LVFU(grbdiff[indx >> 1]) +
                                         p1 * LVFU(grbdiff[(indx - GRBdir1) >>
                                                           1]) +
                                         p2 *
                                             LVFU(grbdiff[((rr - GRBdir0) * ts +
                                                           cc) >>
                                                          1]) +
                                         p3 *
                                             LVFU(grbdiff[((rr - GRBdir0) * ts +
                                ...
                                RBint = vself(vmaskf_gt(vabsf(grbdiffold),
                                                        vabsf(grbdiffint)),
                                              RBint, cinv);
                                RBint = vself(
                                    vmaskf_lt(grbdiffold * grbdiffint, ZEROV),
                                    rinv - zd5v * (grbdiffold + grbdiffint),
                                    RBint);
                            ...
                            for (int c = fc(cfa, rr, cc), indx = rr * ts + cc;
                                ...
                                // G-R/G-B at grid point
                                float grbdiffint =
                                    intp(shiftvfrac[c], grbdiffinthceil,
                                         grbdiffinthfloor);
                                ...
                                if (fabsf(RBint - rgb[c][indx >> 1]) <
                                    ...
                                    grbdiffint =
                                        (p0 * grbdiff[indx >> 1] +
                                         ...
                                         p2 * grbdiff[((rr - GRBdir0) * ts +
                                                       cc) >>
                                                      1] +
                                         p3 * grbdiff[((rr - GRBdir0) * ts +
                                                       cc - GRBdir1) >>
                                                      1]) /
                                        (p0 + p1 + p2 + p3);
                                ...
                                // if colour difference interpolation overshot
                                // the correction, just desaturate
                                if (grbdiffold * grbdiffint < 0) {
                                    rgb[c][indx >> 1] =
                                        rgb[1][indx] -
                                        0.5f * (grbdiffold + grbdiffint);
                                }
                            }
                        }
                        ...
                        for (int rr = border; rr < rr1 - border; rr++) {
                            ...
                            for (; indx <
                                 ...
                                 indx += 4, indx1 += 4) {
                                STVFU(RawDataTmp[indx],
                                      c65535v * LVFU(rgb[c][indx1]));
                            }
#endif
                            for (; indx < (row * width + cc1 - border + left) >>
                                   1;
                                 indx++, indx1++) {
...
#pragma omp for
        ...
        if (avoidColourshift) {
            // to avoid or at least reduce the colour shift caused by raw ca
            // correction we compute the per pixel difference factors of red and
            // blue channel and apply a gaussian blur to them. Then we apply the
            // resulting factors per pixel on the result of raw ca correction
...
#pragma omp for
                ...
                for (int i = 0; i < H - 2 * cb; ++i) {
                    ...
                    for (; j < W - 7 - 2 * cb; j += 8) {
                        ...
                        factors =
                            vself(vmaskf_le(newvals, onev), onev, factors);
                        factors =
                            vself(vmaskf_le(oldvals, onev), onev, factors);
                        STVFU((*nonGreen)[i / 2][j / 2],
                              vclampf(factors, zd5v, twov));
                    }
#endif
                    for (; j < W - 2 * cb; j += 2) {
                        (*nonGreen)[i / 2][j / 2] =
...
#pragma omp single
                ...
                {
                    if (H % 2) {
                        // odd height => factors are not set in last row => use
                        // values of preceding row
                    ...
                    if (W % 2) {
                        // odd width => factors for one channel are not set in
                        // last column => use value of preceding column
